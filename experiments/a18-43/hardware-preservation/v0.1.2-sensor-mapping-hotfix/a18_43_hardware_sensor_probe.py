...
SCHEMA="JANUS/A18.43/hardware-sensor-probe/v0.1.2"
...
def run_probe(output:Path,duration:float,interval:float)->dict[str,Any]:
    ...
    if not first.get('mapping_validation',{}).get('pass'):
        raise RuntimeError('CANONICAL_SENSOR_MAPPING_VALIDATION_FAILED')
    first_identity=json.dumps(first['identity'],sort_keys=True); inventory={'schema':'JANUS/A18.43/sensor-inventory/v0.1.2','generated_at_utc':utc_now(),'system_monitor_lineage':{'repository':'Hawkar-usls/Janus-Demiurge','path':'system_monitor.py','blob_sha':'7deec130091d3fe8c62375e32472326887159f42'},'providers':first['providers'],'canonical_mapping':first['canonical'],'mapping_validation':first.get('mapping_validation')}
    atomic_json(output/'SENSOR_INVENTORY.json',inventory)
    compatibility={'schema':'JANUS/A18.43/system-monitor-compatibility/v0.1.2','status':'PASS','reused':['OpenHardwareMonitor WMI','NVML','nvidia-smi fallback','psutil'],'intentionally_excluded':['CacheProbe','AudioSpectrumAnalyzer','ScreenMonitor','KeyboardMonitor','MouseMonitor','TachyonicRegulator','JanusVault control loop'],'source_blob_sha':'7deec130091d3fe8c62375e32472326887159f42'}
    ...
    while True:
        s=snapshot(); now=time.monotonic(); stable=stable and json.dumps(s['identity'],sort_keys=True)==first_identity and bool(s.get('mapping_validation',{}).get('pass'))
        ...
        record={'schema':'JANUS/A18.43/hardware-sample/v0.1.2','ts_utc':utc_now(),'t_monotonic':now,'sample_index':count,**s}; append_jsonl(samples_path,record); rows.append(flat(s,now)); count+=1

...
SCHEMA = "JANUS/A18.43/preservation-metrics/v0.1.2"
...
def claim_readiness(cap:dict[str,bool], stable_identity:bool, sample_count:int)->dict[str,Any]:
    enough=sample_count>=20 and stable_identity
    ...
    return {'thermal_challenge_ready':thermal,'component_energy_claim_ready':component_energy,'whole_system_energy_claim_ready':whole_system,'fan_wear_metrics_ready':fan,'component_energy_scope':[x for x in ['GPU' if cap.get('has_gpu_power') else None,'CPU_PACKAGE' if cap.get('has_cpu_package_power') else None] if x is not None],'whole_system_energy_block_reason':None if whole_system else 'NO_EXTERNAL_OR_TOTAL_SYSTEM_POWER_SENSOR','hardware_lifetime_claim_ready':False,'hardware_lifetime_claim_block_reason':'REQUIRES_LONGITUDINAL_DEVICE_HEALTH_HISTORY'}
...
def self_test()->int:
    ...
    assert claim_readiness({'has_temperature':True,'has_load':True,'has_cpu_package_power':True,'has_fan':True},True,28)['component_energy_scope']==['CPU_PACKAGE']
    print(json.dumps({'schema':SCHEMA,'status':'PASS','tests':7},sort_keys=True)); return 0

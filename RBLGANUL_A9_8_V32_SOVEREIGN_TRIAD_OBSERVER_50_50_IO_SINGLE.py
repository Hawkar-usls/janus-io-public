...
# -*- coding: utf-8 -*-
"""
RBLGANUL A9.8 / V32 SOVEREIGN TRIAD OBSERVER STRICT 50_50 SINGLE
2026-06-05
...
Run on PC:
  python RBLGANUL_A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_IO_SINGLE.py
...
A9.8 note:
  V32 scheduler/wire behavior is preserved. A9.8 keeps the BunnyHop Memory ReJump
  ...
  A9.8 adds Sovereign Triad as observer-only arbitration labels:
  ...
  Nexus is recorded as a decision label only. It never controls the scheduler.
"""
...
VERSION = "Rblganul A9.8 V32 Sovereign Triad Observer Strict 50_50 IO SINGLE 20260605"
SENTINEL = "RBLGANUL_A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_IO_SINGLE_20260605"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulA9_8"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulA9_8-V32SovereignTriad5050"
...
class A98SovereignTriadAccounting:
    """Fresh-only A9.8 BunnyHop accounting layer plus Sovereign Triad labels.
    ...
    def sovereign_triad(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any, janus: Dict[str, Any], mirror: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if reject_rate > 0.03 or cooldown:
            ...
            winner = "blue_shadow"
        return {
            "schema": "a9-8-sovereign-triad-observer-1",
            "observer_only": True,
            ...
            "decision": {
                "winner": winner,
                "nexus_action": action,
                "meaning": "observer label only; future gates may use this after reproduction, but A9.8 does not",
            },
            "rules": [
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-8-v32-sovereign-triad-observer-strict-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
            "fresh_started_at_utc": self.started_at_utc,
            "fresh_uptime_seconds": max(0.0, time.time() - self.started_wall),
            "objective": "Strict 50/50 comparison plus observer-only Sovereign Triad arbitration labels; BunnyHop/ReJump/WitchHunter behavior remains inherited from A9.7.",
            "wire_change_required": False,
...
class WitchHunter:
    ...
    @staticmethod
    def group_name(lane: Any, strategy: Any = None) -> str:
        return A98SovereignTriadAccounting.group_name(lane, strategy)

    @staticmethod
    def lane_key(lane: Any, strategy: Any, sector: Any, cfg_name: Any) -> str:
        return A98SovereignTriadAccounting.lane_key(lane, strategy, sector, cfg_name)
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-8-witchhunter-dark-tail-observer-1",
...
def sovereign_triad_candidate_labels(cand: Dict[str, Any]) -> Dict[str, Any]:
    ...
    group = A98SovereignTriadAccounting.group_name(lane, strategy)
    face = "gold_sovereign"
    role = "stability_hold"
    ...
    return {
        "schema": "a9-8-sovereign-triad-candidate-label-1",
        "observer_only": True,
        "scheduler_effect": "none",
        "wire_change_required": False,
        "group": group,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA9_8V32SovereignTriadObserver5050")
    ...
    p.add_argument("--csv-log", default="rblganul_a9_8_v32_sovereign_triad_observer_50_50_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a9_8_v32_sovereign_triad_observer_50_50_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a9_8_v32_sovereign_triad_observer_50_50.json", help="periodic/exit A9.8 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a9_8_v32_best_brain.json", help="persistent A9.8 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a9_8_v32_sovereign_triad_observer_50_50_dashboard.json", help="A9.8/V32 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a9_8_v32_zim_stride_memory.json")
    p.add_argument("--disable-notify-oracle", action="store_true")
    p.add_argument("--notify-pause-window-ms", type=int, default=450)
    # V31 DualLock Oracle flags. A9.8 strict mode uses these as internal JANUS-half
    ...
    p.add_argument("--no-strict-50-50-randomized-traversal-mirror", dest="strict_50_50_random_control", action="store_false",
                   help="disable strict split and fall back to weighted A9.8 lane scheduling")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a9_8_v32_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a9_8_v32_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a9_8_v32_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a9_8_v32_sovereign_triad_observer_50_50_accounting.json",
                   help="fresh-only A9.8 strict 50/50 BunnyHop-JANUS-vs-randomized-traversal-mirror accounting dashboard")
    p.add_argument("--disable-a9-accounting", action="store_true",
                   help="disable A9 accounting layer; scheduler/wire are unchanged")
    p.add_argument("--witchhunter-dashboard", default="rblganul_a9_8_v32_witchhunter_dark_tail.json",
                   help="observer-only rejected/stale/drop dark-tail dashboard JSON")
    p.add_argument("--witchhunter-events", default="rblganul_a9_8_v32_witchhunter_dark_tail_events.jsonl",
    ...
    p.add_argument("--enable-bunnyhop-rescout", dest="bunnyhop_rescout_enable", action="store_true", default=True,
                   help="A9.8 default ON: after wake stalls, briefly re-enter JANUS-shaped re-scout instead of staying only in broad mixture")
    p.add_argument("--disable-bunnyhop-rescout", dest="bunnyhop_rescout_enable", action="store_false",
                   help="disable A9.8 Memory ReJump and behave like A9.6 scout/wake")
    ...
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="import older dashboard/session/proof index at startup; default OFF for clean A9.8 evidence")
    ...
    p.add_argument("--io-run-name", default="A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_AFTER_A9_7", help="subfolder for this A9.8 BunnyHop scout/wake run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_AFTER_A9_7") or "A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_AFTER_A9_7")
    ...
    defaults = {
        "csv_log": "rblganul_a9_8_v32_sovereign_triad_observer_50_50_lab.csv",
        "lockbox": "a9_8_v32_sovereign_triad_observer_50_50_lockbox.json",
        "session_summary": "session_summary_a9_8_v32_sovereign_triad_observer_50_50.json",
        "janus_brain": "rblganul_a9_8_v32_best_brain.json",
        "proof_dashboard": "rblganul_a9_8_v32_sovereign_triad_observer_50_50_dashboard.json",
        "stride_memory": "rblganul_a9_8_v32_zim_stride_memory.json",
        "strategy_rates": "rblganul_a9_8_v32_strategy_rates.json",
        "tail_events": "rblganul_a9_8_v32_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a9_8_v32_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a9_8_v32_sovereign_triad_observer_50_50_accounting.json",
        "witchhunter_dashboard": "rblganul_a9_8_v32_witchhunter_dark_tail.json",
        "witchhunter_events": "rblganul_a9_8_v32_witchhunter_dark_tail_events.jsonl",
    }
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A9_8_V32_SOVEREIGN_TRIAD_OBSERVER_50_50_IO_SINGLE.py")
    ...
    A9_ACCOUNTING = None if bool(args.disable_a9_accounting) else A98SovereignTriadAccounting(args.a9_accounting_dashboard)

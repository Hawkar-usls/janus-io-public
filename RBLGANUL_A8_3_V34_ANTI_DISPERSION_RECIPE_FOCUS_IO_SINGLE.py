...
VERSION = "Rblganul A8.3/V34 Anti-Dispersion Recipe Focus IO SINGLE 20260602_RECIPE_FOCUS"
SENTINEL = "RBLGANUL_A8_3_V34_ANTI_DISPERSION_RECIPE_FOCUS_IO_SINGLE_20260602_RECIPE_FOCUS"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulV34-LaneKeyRecipeAudit"
...
class ProtectedExposureReview:
    ...
    def observe_result(self, r: Any) -> None:
        try:
            ...
            ev = {
                ...
                "scheduler_source": str(getattr(r, "scheduler_source", getattr(r, "selected_by", getattr(r, "lane", "unknown")))),
                "proofmind_mode": str(getattr(r, "proofmind_mode", "")),
                ...
                "batch_factor": self._f(getattr(r, "batch_factor", 0.0)),
                "task_id": str(getattr(r, "task_id", "")),
                "arm_weight": getattr(r, "arm_weight", None),
                "arm_reward_ema": getattr(r, "arm_reward_ema", None),
    ...
    def observe_accepted(self, cand: Dict[str, Any], proof_path: str = "") -> None:
        try:
            ...
            ev = {
                ...
                "scheduler_source": str(cand2.get("scheduler_source", cand2.get("selected_by", cand2.get("lane", "unknown")))),
                "proofmind_mode": str(cand2.get("proofmind_mode", "")),
                ...
                "batch_factor": self._f(cand2.get("batch_factor", 0.0)),
                "task_id": str(cand2.get("task_id", "")),
                "arm_weight": cand2.get("arm_weight"),
                "arm_reward_ema": cand2.get("arm_reward_ema"),
    ...
    def observe_rejected(self, cand: Dict[str, Any]) -> None:
        try:
            ...
            ev = {
                ...
                "canonical_key": self._canonical_key(cand.get("strategy", "unknown"), cand.get("sector", 0), cand.get("cfg_name", "canonical")),
                "recipe_key": self._recipe_key(cand.get("lane", "unknown"), cand.get("strategy", "unknown"), cand.get("sector", 0), cand.get("cfg_name", "canonical"), cand.get("worker_id", -1), cand.get("stride_arm", -1), cand.get("stride", 0)),
                "lane": str(cand.get("lane", "unknown")),
                "strategy": str(cand.get("strategy", "unknown")),
                "sector": self._i(cand.get("sector", 0)),
                "worker_id": self._i(cand.get("worker_id", -1)),
                "cfg_name": str(cand.get("cfg_name", "canonical")),
                "stride": self._i(cand.get("stride", 0)),
                "stride_arm": self._i(cand.get("stride_arm", -1)),
                "source_lane": str(cand.get("source_lane", cand.get("lane", "unknown"))),
                "selected_by": str(cand.get("selected_by", cand.get("lane", "unknown"))),
                "scheduler_source": str(cand.get("scheduler_source", cand.get("selected_by", cand.get("lane", "unknown")))),
                "proofmind_mode": str(cand.get("proofmind_mode", "")),
                "recipe_focus": bool(cand.get("recipe_focus", False)),
                "recipe_focus_name": str(cand.get("recipe_focus_name", "")),
                "source_recipe_key": str(cand.get("source_recipe_key", "")),
                "exact_worker_match": bool(cand.get("exact_worker_match", False)),
                "job_age_ms": self._i(cand.get("job_age_ms", 0)),
    ...
    def snapshot(self, shadow: Optional[TachyonShadowMode] = None, client: Optional[Any] = None) -> Dict[str, Any]:
        ...
        # A8.3 gate: review "top" surfaces must be backed by accepted-share
        # evidence. Result-only rows stay in the internal rows, but cannot become
        # operator-facing top candidates.
        top_fresh = [r for r in fresh_rows if int(r.get("fresh_accepted", 0) or 0) > 0][:16]
        top_recipes = [r for r in self._fresh_recipe_rows() if int(r.get("fresh_accepted", 0) or 0) > 0][:32]
...
class RecipeFocusOracle:
    ...
    def __init__(self, enabled: bool = True, focus_share: float = 0.82, a7_weight: float = 0.70, knight_weight: float = 0.25, log_every_rounds: int = 10, exact_worker_only: bool = True) -> None:
    ...
    def _pick_recipe(self, rng: random.Random, recipes: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        pool = recipes or self.recipes
        total = sum(max(0.0, float(r.get("weight", 0.0))) for r in pool)
        if total <= 0:
            return pool[0]
        x = rng.random() * total
        acc = 0.0
    ...
    def choose(self, rng: random.Random, cfgs: List[BuildConfig], round_id: int, worker_id: int) -> Optional[Dict[str, Any]]:
        ...
        if self.exact_worker_only:
            eligible = [r for r in self.recipes if int(worker_id) == int(r.get("worker_id", -999))]
            if not eligible:
                self.last_decision = {
                    "forced": False,
                    "reason": "no_exact_worker_recipe",
                    "round_id": round_id,
                    "worker_id": worker_id,
                ...
                return None
            r = self._pick_recipe(rng, eligible)
        else:
        ...
        self.last_decision = {
            ...
            "stride": r["stride"],
            "focus_share": self.focus_share,
            "exact_worker_only": self.exact_worker_only,
            "allocator": "OFF",
            "wire": "FROZEN",
    ...
    def snapshot(self) -> Dict[str, Any]:
        obj = {
            ...
            "rules": {
                ...
                "exact_worker_only": "default true; non-seed workers fall back to exploration/base scheduler instead of creating sibling recipes",
                "do_not_change": ["header", "nonce", "extranonce2", "ntime", "nbits", "prevhash", "TruthGate", "SubmitGate"],
            },
            "written_at_utc": utc_stamp_iso(),
        }
...
def build_proof(cand: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "miner": {
            ...
            "selected_by": cand.get("selected_by"),
            "scheduler_source": cand.get("scheduler_source"),
            "proofmind_mode": cand.get("proofmind_mode"),
            ...
            "batch_factor": cand.get("batch_factor"),
            "task_id": cand.get("task_id"),
            "arm_weight": cand.get("arm_weight"),
            "arm_reward_ema": cand.get("arm_reward_ema"),
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA8_3V34LaneKeyRecipeAuditOracle")
    ...
    p.add_argument("--csv-log", default="rblganul_a8_3_v34_lanekey_recipe_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a8_3_v34_lanekey_recipe_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a8_3_v34_recipe_focus.json", help="periodic/exit A8.3/V34 protected-exposure session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a8_3_v34_recipe_focus_best_brain.json", help="persistent A8.3/V34 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a8_3_v34_recipe_focus_dashboard.json", help="A8.3/V34 protected-exposure live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a8_3_v34_zim_stride_memory.json")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a8_3_v34_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a8_3_v34_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a8_3_v34_dual_lock_memory.json")
    p.add_argument("--enable-tachyon-shadow", dest="enable_tachyon_shadow", action="store_true", default=True,
    ...
    p.add_argument("--tachyon-shadow-path", default="rblganul_a8_3_v34_tachyon_shadow.json",
    ...
    p.add_argument("--enable-allocator-review", dest="enable_allocator_review", action="store_true", default=True,
    ...
    p.add_argument("--allocator-review-path", default="rblganul_a8_3_v34_allocator_review.json",
    ...
    p.add_argument("--enable-protected-exposure-review", dest="enable_fresh_tailgex_review", action="store_true", default=True,
    ...
    p.add_argument("--protected-exposure-path", "--fresh-tailgex-path", dest="fresh_tailgex_path", default="rblganul_a8_3_v34_recipe_focus_review.json",
    ...
    p.add_argument("--enable-protected-exposure", dest="enable_protected_exposure", action="store_true", default=True,
    ...
    p.add_argument("--enable-recipe-focus", dest="enable_recipe_focus", action="store_true", default=True,
    ...
    p.add_argument("--recipe-focus-exact-worker-only", dest="recipe_focus_exact_worker_only", action="store_true", default=True,
                   help="A8.3 default: only schedule a seed recipe on its historical logical worker")
    p.add_argument("--allow-recipe-sibling-workers", dest="recipe_focus_exact_worker_only", action="store_false",
                   help="Lab mode: allow seed recipes to run on sibling workers; not exact-recipe mode")
    ...
    p.add_argument("--tachyon-import-previous-proofs", dest="tachyon_import_previous_proofs", action="store_true", default=True,
    ...
    p.add_argument("--io-run-name", default="A8_3_V34_ANTI_DISPERSION_RECIPE_FOCUS_AFTER_A8_2", help="subfolder for this A8.3/V34 protected champion exposure run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A8_3_V34_ANTI_DISPERSION_RECIPE_FOCUS_AFTER_A8_2") or "A8_3_V34_ANTI_DISPERSION_RECIPE_FOCUS_AFTER_A8_2")
    ...
    defaults = {
        "csv_log": "rblganul_a8_3_v34_lanekey_recipe_lab.csv",
        "lockbox": "a8_3_v34_lanekey_recipe_lockbox.json",
        "session_summary": "session_summary_a8_3_v34_recipe_focus.json",
        "janus_brain": "rblganul_a8_3_v34_recipe_focus_best_brain.json",
        "proof_dashboard": "rblganul_a8_3_v34_recipe_focus_dashboard.json",
        "stride_memory": "rblganul_a8_3_v34_zim_stride_memory.json",
        "strategy_rates": "rblganul_a8_3_v34_strategy_rates.json",
        "tail_events": "rblganul_a8_3_v34_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a8_3_v34_dual_lock_memory.json",
        "tachyon_shadow_path": "rblganul_a8_3_v34_tachyon_shadow.json",
        "allocator_review_path": "rblganul_a8_3_v34_allocator_review.json",
        "fresh_tailgex_path": "rblganul_a8_3_v34_recipe_focus_review.json",
    }
    ...
    try:
        atomic_json(str(run_dir / "a8_3_v34_io_bootstrap.json"), info)
    except Exception:
        pass
    ...
    for cand in candidates:
        ...
        if V34_DUPLICATE_SUBMIT_GUARD is not None:
            ...
            if not allowed:
                ...
                dump_json("rblganul_a8_3_v34_duplicate_submit_drop.json", cand)
                continue
...
def main() -> None:
    ...
    source_sha16 = ""
    source_path = str(io_info.get("script_path") or os.path.abspath(sys.argv[0]))
    ...
    log("io", f"source_file={source_path}")
    ...
    log("run", "no long command needed: python RBLGANUL_A8_3_V34_ANTI_DISPERSION_RECIPE_FOCUS_IO_SINGLE.py")
    ...
    log("recipe_focus", f"A8.3 Anti-Dispersion Recipe Focus enabled={getattr(args, 'enable_recipe_focus', True)} allocator=OFF wire=FROZEN focus_share={getattr(args, 'recipe_focus_share', 0.82):.2f} exact_worker_only={getattr(args, 'recipe_focus_exact_worker_only', True)} A7=w9/a11/stride521 knight=w1/a-1/stride0")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("recipe_focus", f"enable_recipe_focus={args.enable_recipe_focus} focus_share={args.recipe_focus_share} exact_worker_only={args.recipe_focus_exact_worker_only} a7_weight={args.recipe_focus_a7_weight} knight_weight={args.recipe_focus_knight_weight}")
        log("io", f"bootstrap={Path(args.lockbox).parent / 'a8_3_v34_io_bootstrap.json'}")
        return
    ...
    V34_RECIPE_FOCUS = RecipeFocusOracle(
        ...
        exact_worker_only=getattr(args, "recipe_focus_exact_worker_only", True),
    )

...
# -*- coding: utf-8 -*-
"""
RBLGANUL A10.2 / V34 FULL PHANTOM POOLDAY NETPULSE SINGLE
2026-05-27
...
Run on PC:
  python RBLGANUL_A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_IO_SINGLE.py
...
Full Phantom note:
  V32 scheduler/wire behavior is preserved. A10.2 keeps equal-exposure 50/50
  accounting, PoolDay, Ramanujan theta telemetry, and adds NetPulse telemetry:
  Stratum RTT, packet byte/message counters, submit-ack latency, socket idle,
  notify cadence, and pending-submit pressure. These observers do not apply
  allocator weights or change submit behavior.
"""
...
VERSION = "Rblganul A10.2 V34 Full Phantom PoolDay NetPulse IO SINGLE 20260604"
SENTINEL = "RBLGANUL_A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_IO_SINGLE_20260604"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulPhantomNetPulse"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulPhantomNetPulse-Observer50"
...
def target_to_difficulty(target: int) -> float:
    if target <= 0:
        return float("inf")
    try:
        return float(decimal.Decimal(DIFF1_TARGET) / decimal.Decimal(int(target)))
    except Exception:
        return 0.0


def extract_bip34_height_from_coinb1(coinb1_hex: str) -> int:
    """Best-effort BIP34 height sniff from coinbase prefix; read-only telemetry."""
    try:
        data = bytes.fromhex(str(coinb1_hex or ""))
    except Exception:
        return 0
    best = 0
    for i in range(0, max(0, len(data) - 6)):
        ln = data[i]
        if ln < 2 or ln > 5 or i + 1 + ln > len(data):
            continue
        val = int.from_bytes(data[i + 1:i + 1 + ln], "little", signed=False)
        # Keep this broad enough for future Bitcoin heights but reject script noise.
        if 500_000 <= val <= 5_000_000:
            best = val
    return best


class PoolDayThetaContext:
    """PoolDay + Ramanujan theta phase layer.

    Observe-only with one optional scheduler use on the JANUS half. It never
    changes nonce/header/extranonce/prevhash/submit wire.
    """

    def __init__(self, theta_terms: int = 12) -> None:
        self.theta_terms = max(4, min(48, int(theta_terms or 12)))
        self.started_at_utc = utc_stamp_iso()
        self.last_job_id = ""
        self.last_job_received_ms = 0
        self.job_cadence_ms = 0
        self.clean_window: Deque[int] = deque(maxlen=128)
        self.last_snapshot: Dict[str, Any] = {}

    @staticmethod
    def theta2(q: float, terms: int) -> float:
        q = max(1e-9, min(0.999999, float(q)))
        return 2.0 * sum(q ** ((n + 0.5) * (n + 0.5)) for n in range(terms))

    @staticmethod
    def theta3(q: float, terms: int) -> float:
        q = max(1e-9, min(0.999999, float(q)))
        return 1.0 + 2.0 * sum(q ** (n * n) for n in range(1, terms + 1))

    @staticmethod
    def theta4(q: float, terms: int) -> float:
        q = max(1e-9, min(0.999999, float(q)))
        return 1.0 + 2.0 * sum(((-1.0) ** n) * (q ** (n * n)) for n in range(1, terms + 1))

    @staticmethod
    def mode_from_snapshot(snap: Dict[str, Any]) -> str:
        netpulse = snap.get("netpulse") if isinstance(snap.get("netpulse"), dict) else {}
        if str(netpulse.get("network_pressure", "OK")) in ("HIGH", "ELEVATED"):
            return "NETWORK_PRESSURE"
        if bool(snap.get("clean")) and float(snap.get("job_age_ms", 0.0) or 0.0) < 2500.0:
            return "FRESH_CLEAN_JOB"
        if float(snap.get("job_age_ms", 0.0) or 0.0) > 75_000.0:
            return "AGED_JOB"
        progress = float(snap.get("retarget_progress", 0.0) or 0.0)
        if progress < 0.03 or progress > 0.97:
            return "RETARGET_EDGE"
        if float(snap.get("clean_job_rate", 0.0) or 0.0) > 0.65:
            return "CLEAN_JOB_STREAM"
        return "POOLDAY_NORMAL"

    def snapshot(self, job: "Job", pool_diff: float, round_id: int, netpulse: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        now = now_ms()
        if job.job_id != self.last_job_id:
            if self.last_job_received_ms > 0:
                self.job_cadence_ms = max(0, int(job.received_ms) - int(self.last_job_received_ms))
            self.last_job_id = job.job_id
            self.last_job_received_ms = int(job.received_ms or now)
            self.clean_window.append(1 if bool(job.clean) else 0)
        height = extract_bip34_height_from_coinb1(job.coinb1)
        net_target = compact_bits_to_target(job.nbits)
        net_diff = target_to_difficulty(net_target) if net_target else 0.0
        retarget_mod = height % 2016 if height > 0 else 0
        retarget_progress = (float(retarget_mod) / 2016.0) if height > 0 else 0.0
        job_age_ms = max(0, int(now) - int(job.received_ms or now))
        q_seed = stable_seed(job.job_id, job.seq, job.nbits, job.ntime, height, round_id) % 997
        q = 0.21 + 0.72 * (float(q_seed) / 996.0)
        t2 = self.theta2(q, self.theta_terms)
        t3 = self.theta3(q, self.theta_terms)
        t4 = self.theta4(q, self.theta_terms)
        theta_mix = (t2 * 0.37) + (t3 * 0.41) + (abs(t4) * 0.22)
        theta_bucket = int(abs(theta_mix * 1000.0)) % 8
        q_bucket = int(q * 1000.0) % 16
        clean_rate = sum(self.clean_window) / max(1, len(self.clean_window))
        snap = {
            "enabled": True,
            "job_id": job.job_id,
            "job_seq": int(job.seq),
            ...
            "height": int(height),
            "height_mod_2016": int(retarget_mod),
            "retarget_progress": retarget_progress,
            "nbits": job.nbits,
            "ntime": job.ntime,
            "network_difficulty_from_nbits": net_diff,
            "implied_network_hashrate_ehps": (net_diff * (2.0 ** 32) / 600.0) / 1e18 if net_diff > 0 and math.isfinite(net_diff) else 0.0,
            "pool_diff": float(pool_diff or 0.0),
            "job_age_ms": int(job_age_ms),
            "job_cadence_ms": int(self.job_cadence_ms),
            "clean": bool(job.clean),
            "clean_job_rate": clean_rate,
            "q": q,
            "q_bucket": int(q_bucket),
            "theta_terms": int(self.theta_terms),
            "theta2": t2,
            "theta3": t3,
            "theta4": t4,
            "theta_bucket": int(theta_bucket),
            "netpulse": dict(netpulse or {}),
            "submit_ack_rtt_ms": float((netpulse or {}).get("submit_ack_last_ms", 0.0) or 0.0),
            "submit_ack_avg_ms": float((netpulse or {}).get("submit_ack_avg_ms", 0.0) or 0.0),
            "rpc_rtt_avg_ms": float((netpulse or {}).get("rpc_rtt_avg_ms", 0.0) or 0.0),
            "socket_idle_ms": int((netpulse or {}).get("socket_idle_ms", 0) or 0),
            "read_loop_gap_ms": int((netpulse or {}).get("read_loop_gap_ms", 0) or 0),
            "pending_submit_age_max_ms": float((netpulse or {}).get("pending_submit_age_max_ms", 0.0) or 0.0),
            "network_pressure": str((netpulse or {}).get("network_pressure", "OK")),
        }
        snap["poolday_mode"] = self.mode_from_snapshot(snap)
        self.last_snapshot = snap
        return snap


def phantom_theta_choice(base: Tuple[str, int, BuildConfig, str], snap: Dict[str, Any], rng: random.Random, cfg: BuildConfig) -> Tuple[str, int, BuildConfig, str]:
    """Guide only the JANUS half; random_control never passes through here."""
    st, sec, cfg2, lane = base
    mode = str(snap.get("poolday_mode", "POOLDAY_NORMAL"))
    bucket = int(snap.get("theta_bucket", 0) or 0) % 8
    height_mod = int(snap.get("height_mod_2016", 0) or 0)
    if mode in ("FRESH_CLEAN_JOB", "CLEAN_JOB_STREAM") and bucket in (0, 3, 6):
        return "zim_reverse", 6, cfg, f"phantom_theta:zim_s6:b{bucket}"
    if mode == "AGED_JOB" and bucket in (1, 5):
        return "linear", (height_mod + bucket) % SECTORS, cfg, f"phantom_theta:linear:b{bucket}"
    if bucket == 7:
        return "knight", 11, cfg, "phantom_theta:knight_s11:b7"
    if bucket in (2, 4) and rng.random() < 0.50:
        return "linear", (height_mod + sec + bucket) % SECTORS, cfg, f"phantom_theta:linear:b{bucket}"
    return st, sec, cfg2, f"phantom_janus:{lane}"
...
class StratumClient:
    def __init__(self, host: str, port: int, use_tls: bool, user: str, password: str, subscribe_tag: str = DEFAULT_SUBSCRIBE_TAG, proof_dir: str = "proofs"):
        ...
        self.netpulse_started_wall = time.time()
        self.netpulse_started_utc = utc_stamp_iso()
        self.connect_rtt_ms = 0.0
        self.last_send_ms = 0
        self.last_recv_ms = 0
        self.last_read_call_ms = 0
        self.last_read_gap_ms = 0
        self.last_notify_ms = 0
        self.last_notify_gap_ms = 0
        self.read_calls = 0
        self.read_empty_count = 0
        self.response_timeout_count = 0
        self.method_out: Dict[str, int] = defaultdict(int)
        self.method_in: Dict[str, int] = defaultdict(int)
        self.rpc_rtt_recent: Deque[float] = deque(maxlen=128)
        self.submit_ack_recent: Deque[float] = deque(maxlen=128)
        self.last_rpc_rtt_ms = 0.0
        self.last_submit_ack_ms = 0.0
        self.last_subscribe_rtt_ms = 0.0
        self.last_authorize_rtt_ms = 0.0
        self.last_suggest_diff_rtt_ms = 0.0
    ...
    def _record_rpc_response(self, mid: Any) -> Tuple[str, float]:
        req = self.pending_requests.pop(int(mid), None) if isinstance(mid, int) else None
        if not req:
            return "", 0.0
        rtt = max(0.0, float(now_ms() - int(req.get("sent_ms", now_ms()))))
        method = str(req.get("method", ""))
        self.last_rpc_rtt_ms = rtt
        self.rpc_rtt_recent.append(rtt)
        if method == "mining.subscribe":
            self.last_subscribe_rtt_ms = rtt
        elif method == "mining.authorize":
            self.last_authorize_rtt_ms = rtt
        elif method == "mining.suggest_difficulty":
            self.last_suggest_diff_rtt_ms = rtt
        elif method == "mining.submit":
            self.last_submit_ack_ms = rtt
            self.submit_ack_recent.append(rtt)
        return method, rtt

    def netpulse_snapshot(self) -> Dict[str, Any]:
        now = now_ms()
        pending_ages = [max(0.0, float(now - int(c.get("netpulse_submit_sent_ms", now)))) for c in self.pending_submits.values()]
        pending_rpc_items = list(self.pending_requests.values())
        optional_rpc_methods = {"mining.suggest_difficulty"}
        pending_rpc_ages = [max(0.0, float(now - int(r.get("sent_ms", now)))) for r in pending_rpc_items]
        pending_blocking_rpc_ages = [
            max(0.0, float(now - int(r.get("sent_ms", now))))
            for r in pending_rpc_items
            if str(r.get("method", "")) not in optional_rpc_methods
        ]
        pending_optional_rpc_ages = [
            max(0.0, float(now - int(r.get("sent_ms", now))))
            for r in pending_rpc_items
            if str(r.get("method", "")) in optional_rpc_methods
        ]
        submit_avg = self._avg(self.submit_ack_recent)
        rpc_avg = self._avg(self.rpc_rtt_recent)
        socket_idle = max(0, int(now - self.last_recv_ms)) if self.last_recv_ms else 0
        pressure = "OK"
        if self.response_timeout_count > 0 or socket_idle > 30_000 or self._max(pending_ages) > 10_000 or self._max(pending_blocking_rpc_ages) > 15_000:
            pressure = "HIGH"
        elif submit_avg > 1500 or self.last_read_gap_ms > 5000 or socket_idle > 10_000:
            pressure = "ELEVATED"
        return {
            "schema": "a10-2-netpulse-stratum-1",
            "started_at_utc": self.netpulse_started_utc,
            "uptime_seconds": max(0.0, time.time() - self.netpulse_started_wall),
            "host": self.host,
            "port": self.port,
            "tls": bool(self.use_tls),
            "connect_rtt_ms": self.connect_rtt_ms,
            "subscribe_rtt_ms": self.last_subscribe_rtt_ms,
            "authorize_rtt_ms": self.last_authorize_rtt_ms,
            "suggest_diff_rtt_ms": self.last_suggest_diff_rtt_ms,
            "rpc_rtt_last_ms": self.last_rpc_rtt_ms,
            "rpc_rtt_avg_ms": rpc_avg,
            "rpc_rtt_max_recent_ms": self._max(self.rpc_rtt_recent),
            "submit_ack_last_ms": self.last_submit_ack_ms,
            "submit_ack_avg_ms": submit_avg,
            "submit_ack_max_recent_ms": self._max(self.submit_ack_recent),
            "read_calls": int(self.read_calls),
            "read_empty_count": int(self.read_empty_count),
            "response_timeout_count": int(self.response_timeout_count),
            "read_loop_gap_ms": int(self.last_read_gap_ms),
            "socket_idle_ms": int(socket_idle),
            "last_notify_gap_ms": int(self.last_notify_gap_ms),
            "pending_submit_count": len(self.pending_submits),
            "pending_submit_age_avg_ms": self._avg(pending_ages),
            "pending_submit_age_max_ms": self._max(pending_ages),
            "pending_rpc_count": len(self.pending_requests),
            "pending_rpc_age_avg_ms": self._avg(pending_rpc_ages),
            "pending_rpc_age_max_ms": self._max(pending_rpc_ages),
            "pending_blocking_rpc_count": len(pending_blocking_rpc_ages),
            "pending_blocking_rpc_age_max_ms": self._max(pending_blocking_rpc_ages),
            "pending_optional_rpc_count": len(pending_optional_rpc_ages),
            "pending_optional_rpc_age_max_ms": self._max(pending_optional_rpc_ages),
            "optional_rpc_note": "mining.suggest_difficulty can be one-way on some pools; it is not counted as network pressure",
            "bytes_in": int(getattr(self.sock, "bytes_in", 0)),
            "bytes_out": int(getattr(self.sock, "bytes_out", 0)),
            "messages_in": int(getattr(self.sock, "messages_in", 0)),
            "messages_out": int(getattr(self.sock, "messages_out", 0)),
            "method_out": dict(self.method_out),
            "method_in": dict(self.method_in),
            "network_pressure": pressure,
            "wire_change_required": False,
        }

    def netpulse_line(self) -> str:
        snap = self.netpulse_snapshot()
        return (
            f"pressure={snap.get('network_pressure')} submit_ack_avg={float(snap.get('submit_ack_avg_ms',0.0)):.1f}ms "
            f"rpc_avg={float(snap.get('rpc_rtt_avg_ms',0.0)):.1f}ms idle={snap.get('socket_idle_ms')}ms "
            f"pending={snap.get('pending_submit_count')} bytes_in={snap.get('bytes_in')} bytes_out={snap.get('bytes_out')}"
        )
    ...
    def connect(self) -> None:
        ...
        t0 = now_ms()
        self.sock.connect()
        self.connect_rtt_ms = max(0.0, float(now_ms() - t0))
    ...
    def send(self, method: str, params: List[Any]) -> int:
        ...
        self.pending_requests[msg_id] = {"method": method, "sent_ms": sent_ms}
        self.method_out[method] += 1
        self.last_send_ms = sent_ms
        return msg_id
    ...
    def submit(self, cand: Dict[str, Any]) -> int:
        ...
        cand["netpulse_submit_id"] = msg_id
        cand["netpulse_submit_sent_ms"] = now_ms()
    ...
    def handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        if method == "mining.notify":
            ...
            try:
                ...
                if self.last_notify_ms:
                    self.last_notify_gap_ms = max(0, int(self.job.received_ms - self.last_notify_ms))
                self.last_notify_ms = int(self.job.received_ms)
        ...
        if "id" in msg:
            mid = msg.get("id")
            response_method, response_rtt_ms = self._record_rpc_response(mid)
            if mid in self.pending_submits:
                cand = self.pending_submits.pop(mid)
                cand["netpulse_response_method"] = response_method
                cand["netpulse_submit_ack_ms"] = response_rtt_ms
                cand["netpulse_snapshot"] = self.netpulse_snapshot()
                if msg.get("result") is True:
                    ...
                    try:
                        ...
                        if PHANTOM_OBSERVER_PACK is not None:
                            PHANTOM_OBSERVER_PACK.observe_rejected(cand2)
                    except Exception as e:
    ...
    def read_available(self, max_messages: int = 100, timeout: float = 0.0) -> int:
        now = now_ms()
        if self.last_read_call_ms:
            self.last_read_gap_ms = max(0, int(now - self.last_read_call_ms))
        self.last_read_call_ms = int(now)
        self.read_calls += 1
        n = 0
...
class MineTask:
    ...
    stride: int = 0
    stride_arm: int = -1
    poolday: Optional[Dict[str, Any]] = None
            ...
            if target_pass or z >= task.signal_z:
                if target_pass and local_submit_count < task.submit_limit:
                    ...
                    cand = {
                        ...
                        "poolday": dict(task.poolday or {}),
                        "theta_bucket": (task.poolday or {}).get("theta_bucket"),
                        "q_bucket": (task.poolday or {}).get("q_bucket"),
                        "poolday_mode": (task.poolday or {}).get("poolday_mode"),
...
class PhantomPoolDayThetaAccounting:
    """Fresh-only Phantom accounting layer.

    This class does not choose tasks and does not touch header/nonce/submit wire.
    It groups equal-exposure tasks into:
      - janus_phantom: PoolDay/Theta-guided JANUS traversal half
      - random_control: explicit random_baseline half
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        self.poolday_modes: Dict[str, Dict[str, Any]] = {}
        self.theta_buckets: Dict[str, Dict[str, Any]] = {}
    ...
    def _poolday_row(self, mode: str) -> Dict[str, Any]:
        name = str(mode or "POOLDAY_UNKNOWN")
        if name not in self.poolday_modes:
            self.poolday_modes[name] = self._base_row(name, "poolday_mode")
        return self.poolday_modes[name]

    def _theta_row(self, bucket: Any) -> Dict[str, Any]:
        try:
            name = f"theta_b{int(bucket or 0) % 8}"
        except Exception:
            name = "theta_unknown"
        if name not in self.theta_buckets:
            self.theta_buckets[name] = self._base_row(name, "theta_bucket")
        return self.theta_buckets[name]
    ...
    def observe_result(self, r: Any) -> None:
        ...
        poolday = getattr(r, "poolday", {}) or {}
        checked = int(getattr(r, "checked", 0) or 0)
        best_z = int(getattr(r, "best_z", 0) or 0)
        hps = float(getattr(r, "hps", 0.0) or 0.0)
        rows = [self._group_row(group), self._lane_row(key), self._load_row(self.current_load_state)]
        if group == "janus_phantom":
            rows.append(self._poolday_row(str(poolday.get("poolday_mode", "POOLDAY_UNKNOWN"))))
            rows.append(self._theta_row(poolday.get("theta_bucket", 0)))
        for row in rows:
    ...
    def _observe_submit(self, cand: Dict[str, Any], accepted: bool) -> None:
        ...
        poolday = cand.get("poolday") if isinstance(cand.get("poolday"), dict) else {}
        zbits = int(cand.get("zbits", 0) or 0)
        rows = [self._group_row(group), self._lane_row(key), self._load_row(self.current_load_state)]
        if group == "janus_phantom":
            rows.append(self._poolday_row(str(poolday.get("poolday_mode", cand.get("poolday_mode", "POOLDAY_UNKNOWN")))))
            rows.append(self._theta_row(poolday.get("theta_bucket", cand.get("theta_bucket", 0))))
        for row in rows:
        ...
        if accepted and zbits >= 30:
            self.events.append({
                ...
                "poolday_mode": poolday.get("poolday_mode", cand.get("poolday_mode")),
                "theta_bucket": poolday.get("theta_bucket", cand.get("theta_bucket")),
                "q_bucket": poolday.get("q_bucket", cand.get("q_bucket")),
                "zbits": zbits,
                "job_id": cand.get("job_id"),
                "nonce_submit_hex": cand.get("nonce_submit_hex"),
            })
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        poolday_modes = {k: self._decorated_row(v) for k, v in self.poolday_modes.items()}
        theta_buckets = {k: self._decorated_row(v) for k, v in self.theta_buckets.items()}
        lanes.sort(key=lambda r: (int(r.get("best_z", 0)), float(r.get("accepted_per_mh", 0.0)), int(r.get("accepted", 0))), reverse=True)
        janus = groups.get("janus_phantom", self._decorated_row(self._base_row("janus_phantom")))
        random_control = groups.get("random_control", self._decorated_row(self._base_row("random_control")))
        advantage: Dict[str, Any] = {}
        for z in (28, 30, 32, 33, 34, 36, 38):
            key = f"z{z}"
            j = float((janus.get("accepted_tail_per_mh") or {}).get(key, 0.0) or 0.0)
            r = float((random_control.get("accepted_tail_per_mh") or {}).get(key, 0.0) or 0.0)
            advantage[key] = {
                "janus_accepted_per_mh": j,
                "random_control_accepted_per_mh": r,
                "ratio": (j / r if r > 0 else None),
            }
        random_mh = float(random_control.get("mh", 0.0) or 0.0)
        verdict = "NEED_MORE_RANDOM_CONTROL"
        if random_mh >= float(getattr(args, "a9_min_random_control_mh", 250.0) or 250.0):
            verdict = "READY_FOR_COMPARISON"
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a10-2-v34-phantom-poolday-theta-netpulse-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
            "fresh_started_at_utc": self.started_at_utc,
            "fresh_uptime_seconds": max(0.0, time.time() - self.started_wall),
            "objective": "Compare Phantom PoolDay/Theta-guided JANUS traversal against explicit random_baseline with equal 50/50 worker exposure.",
            "wire_change_required": False,
            "engine_behavior_changed": False,
            "phantom_50_50": bool(getattr(args, "phantom_50_50", True)),
            "poolday_enabled": not bool(getattr(args, "disable_poolday", False)),
            "theta_enabled": not bool(getattr(args, "disable_theta", False)),
            "current_runtime": dict(self.last_runtime),
            "last_poolday": dict(getattr(PHANTOM_CONTEXT, "last_snapshot", {}) or {}),
            "netpulse": client.netpulse_snapshot() if client is not None and hasattr(client, "netpulse_snapshot") else {},
            "health": {
                ...
                "network_pressure": (client.netpulse_snapshot() if client is not None and hasattr(client, "netpulse_snapshot") else {}).get("network_pressure", "UNKNOWN"),
            },
            ...
            "load_states": loads,
            "poolday_modes": poolday_modes,
            "theta_buckets": theta_buckets,
            "top_lanes": lanes[:20],
            "recent_tail_events": list(self.events),
            "comparison_gate": {
                "verdict": verdict,
                "random_control_mh": random_mh,
    ...
    def write_dashboard(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> None:
        try:
            ...
            log("phantom", f"accounting dashboard write failed: {e}")

    def line(self) -> str:
        janus = self._decorated_row(self.groups.get("janus_phantom", self._base_row("janus_phantom")))
        random_control = self._decorated_row(self.groups.get("random_control", self._base_row("random_control")))
        j32 = (janus.get("accepted_tail_per_mh") or {}).get("z32", 0.0)
        r32 = (random_control.get("accepted_tail_per_mh") or {}).get("z32", 0.0)
        snap = getattr(PHANTOM_CONTEXT, "last_snapshot", {}) or {}
        return (
            f"state={self.current_load_state} "
            f"janus_mh={janus.get('mh', 0.0):.1f} random_mh={random_control.get('mh', 0.0):.1f} "
            f"janus_z32/MH={j32:.4f} random_z32/MH={r32:.4f} "
            f"poolday={snap.get('poolday_mode', 'n/a')} theta_b={snap.get('theta_bucket', 'n/a')}"
        )


class FullPhantomObserverPack:
    """A10.2 observer pack.

    This class only records diagnostics. It does not choose tasks, does not
    mutate scheduler weights, and does not touch header/nonce/submit wire.
    """
    ...
    def __init__(self, path: str) -> None:
        ...
        self.seen_submit_keys: Dict[str, str] = {}
        self.recent_events: Deque[Dict[str, Any]] = deque(maxlen=256)

    @staticmethod
    def group_name(lane: Any) -> str:
        return "random_control" if str(lane or "") == "random_baseline" else "janus_phantom"
    ...
    @staticmethod
    def recipe_key_from_obj(obj: Any) -> str:
        getter = obj.get if isinstance(obj, dict) else lambda k, d=None: getattr(obj, k, d)
        existing = getter("recipe_key", "")
        if existing:
            return str(existing)
        try:
            sec = int(getter("sector", 0) or 0) % SECTORS
        except Exception:
            sec = 0
        lane = getter("lane", "unknown")
        strategy = getter("strategy", "unknown")
        cfg = getter("cfg_name", "canonical")
        worker_id = getter("worker_id", -1)
        stride_arm = getter("stride_arm", -1)
        stride = int(getter("stride", 0) or 0) & 0xFFFFFFFF
        return f"{lane}/{strategy}/s{sec}/{cfg}/w{worker_id}/a{stride_arm}/stride{stride}"
    ...
    def _row(self, table: Dict[str, Dict[str, Any]], key: str, kind: str) -> Dict[str, Any]:
        if key not in table:
            table[key] = self._base_row(key, kind)
        return table[key]

    def _result_rows(self, r: Any) -> List[Dict[str, Any]]:
        lane = getattr(r, "lane", "unknown")
        strategy = getattr(r, "strategy", "unknown")
        sector = getattr(r, "sector", 0)
        cfg_name = getattr(r, "cfg_name", "canonical")
        poolday = getattr(r, "poolday", {}) or {}
        rows = [
            self._row(self.groups, self.group_name(lane), "group"),
            self._row(self.lanes, self.lane_key(lane, strategy, sector, cfg_name), "lane"),
            self._row(self.recipes, self.recipe_key_from_obj(r), "recipe"),
        ]
        mode = str(poolday.get("poolday_mode", "POOLDAY_UNKNOWN"))
        rows.append(self._row(self.poolday_modes, f"{self.group_name(lane)}::{mode}", "poolday_mode"))
        if self.group_name(lane) == "janus_phantom":
            try:
                tb = int(poolday.get("theta_bucket", 0) or 0) % 8
            except Exception:
                tb = -1
            rows.append(self._row(self.theta_buckets, f"theta_b{tb}", "theta_bucket"))
        return rows

    def _submit_rows(self, cand: Dict[str, Any]) -> List[Dict[str, Any]]:
        lane = cand.get("lane", "unknown")
        strategy = cand.get("strategy", "unknown")
        sector = cand.get("sector", 0)
        cfg_name = cand.get("cfg_name", "canonical")
        poolday = cand.get("poolday") if isinstance(cand.get("poolday"), dict) else {}
        rows = [
            self._row(self.groups, self.group_name(lane), "group"),
            self._row(self.lanes, self.lane_key(lane, strategy, sector, cfg_name), "lane"),
            self._row(self.recipes, self.recipe_key_from_obj(cand), "recipe"),
        ]
        mode = str(poolday.get("poolday_mode", cand.get("poolday_mode", "POOLDAY_UNKNOWN")))
        rows.append(self._row(self.poolday_modes, f"{self.group_name(lane)}::{mode}", "poolday_mode"))
        if self.group_name(lane) == "janus_phantom":
            try:
                tb = int(poolday.get("theta_bucket", cand.get("theta_bucket", 0)) or 0) % 8
            except Exception:
                tb = -1
            rows.append(self._row(self.theta_buckets, f"theta_b{tb}", "theta_bucket"))
        return rows

    def _touch_result(self, row: Dict[str, Any], checked: int, best_z: int) -> None:
        row["checked"] = int(row.get("checked", 0)) + max(0, int(checked or 0))
        row["result_count"] = int(row.get("result_count", 0)) + 1
        row["best_z"] = max(int(row.get("best_z", 0)), int(best_z or 0))
        tails = row.setdefault("result_best_tails", {f"z{z}": 0 for z in self.TAILS})
    ...
    def _touch_submit(self, row: Dict[str, Any], zbits: int, accepted: bool) -> None:
        if accepted:
            row["accepted"] = int(row.get("accepted", 0)) + 1
            row["submitted"] = int(row.get("submitted", 0)) + 1
            row["best_z"] = max(int(row.get("best_z", 0)), int(zbits or 0))
    ...
    def observe_result(self, r: Any) -> None:
        checked = int(getattr(r, "checked", 0) or 0)
        best_z = int(getattr(r, "best_z", 0) or 0)
        for row in self._result_rows(r):
            self._touch_result(row, checked, best_z)

    def _submit_key(self, cand: Dict[str, Any]) -> str:
        return "|".join(str(cand.get(k, "")) for k in ("job_id", "extranonce2", "ntime", "nonce_submit_hex"))

    def _observe_duplicate_risk(self, cand: Dict[str, Any], accepted: bool) -> None:
        key = self._submit_key(cand)
        if not key:
            return
        previous = self.seen_submit_keys.get(key)
        if previous is not None:
            row = self.duplicates.setdefault(key, {
                "key": key,
                "first_seen_utc": previous,
                "last_seen_utc": "",
                "count": 1,
                "accepted": 0,
                "rejected": 0,
            ...
            row["accepted"] = int(row.get("accepted", 0)) + (1 if accepted else 0)
            row["rejected"] = int(row.get("rejected", 0)) + (0 if accepted else 1)
            row["last_seen_utc"] = utc_stamp_iso()
            return
        self.seen_submit_keys[key] = utc_stamp_iso()
    ...
    def _observe_submit(self, cand: Dict[str, Any], accepted: bool) -> None:
        zbits = int(cand.get("zbits", 0) or 0)
        self._observe_duplicate_risk(cand, accepted)
        for row in self._submit_rows(cand):
            self._touch_submit(row, zbits, accepted)
        if accepted and zbits >= 28:
            poolday = cand.get("poolday") if isinstance(cand.get("poolday"), dict) else {}
            self.recent_events.append({
                "ts_utc": utc_stamp_iso(),
                "zbits": zbits,
                "group": self.group_name(cand.get("lane")),
                "lane": cand.get("lane"),
                "recipe_key": self.recipe_key_from_obj(cand),
                "poolday_mode": cand.get("poolday_mode") or poolday.get("poolday_mode"),
                "theta_bucket": cand.get("theta_bucket") if cand.get("theta_bucket") is not None else poolday.get("theta_bucket"),
                "nonce_submit_hex": cand.get("nonce_submit_hex"),
            })
    ...
    def _decorate(self, row: Dict[str, Any]) -> Dict[str, Any]:
        out = dict(row)
        mh = max(0.0, float(out.get("checked", 0) or 0) / 1_000_000.0)
        acc = int(out.get("accepted", 0) or 0)
        out["mh"] = mh
        out["accepted_per_mh"] = float(acc) / mh if mh > 0 else 0.0
        out["reject_rate"] = float(out.get("rejected", 0) or 0) / max(1.0, float(out.get("submitted", 0) or 0))
        tails = dict(out.get("accepted_tails", {}) or {})
        out["accepted_tail_per_mh"] = {k: (float(v or 0) / mh if mh > 0 else 0.0) for k, v in tails.items()}
        gex = 0.0
        components = {}
        for z in (28, 30, 32, 33, 34, 36, 38):
            actual = int(tails.get(f"z{z}", 0) or 0)
            expected = float(acc) / math.pow(2.0, z - 23) if acc > 0 else 0.0
            signal = (actual - expected) / math.sqrt(expected + 1.0)
            components[f"z{z}"] = {"actual": actual, "expected": expected, "signal": signal}
            gex += signal
        out["fresh_tailgex"] = gex
        out["fresh_tailgex_components"] = components
        out["observer_trusted"] = acc >= 50 and mh >= 250.0
        return out

    def summary(self) -> Dict[str, Any]:
        groups = {k: self._decorate(v) for k, v in self.groups.items()}
        lanes = [self._decorate(v) for v in self.lanes.values()]
        recipes = [self._decorate(v) for v in self.recipes.values()]
        poolday_modes = [self._decorate(v) for v in self.poolday_modes.values()]
        theta_buckets = [self._decorate(v) for v in self.theta_buckets.values()]
        keyfn = lambda r: (float(r.get("fresh_tailgex", 0.0)), int(r.get("best_z", 0)), int(r.get("accepted", 0)), float(r.get("mh", 0.0)))
        lanes.sort(key=keyfn, reverse=True)
        recipes.sort(key=keyfn, reverse=True)
        poolday_modes.sort(key=keyfn, reverse=True)
        theta_buckets.sort(key=keyfn, reverse=True)
        dup_rows = sorted(self.duplicates.values(), key=lambda r: int(r.get("count", 0)), reverse=True)
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a10-2-v34-full-phantom-netpulse-observer-pack-1",
            "written_at_utc": utc_stamp_iso(),
            "fresh_started_at_utc": self.started_at_utc,
            "fresh_uptime_seconds": max(0.0, time.time() - self.started_wall),
            "wire_change_required": False,
            ...
            "observer_pack": {
                "tachyon_shadow_fresh_only": True,
                "fresh_tailgex": True,
                "exact_recipe_scoreboard": True,
                "exposure_watch": True,
                "duplicate_submit_watch": True,
            },
            "groups": groups,
            "top_lanes": lanes[:32],
            "top_exact_recipes": recipes[:32],
            "poolday_modes": poolday_modes[:32],
            "theta_buckets": theta_buckets[:16],
            "duplicate_submit_watch": {
                "seen_submit_keys": len(self.seen_submit_keys),
                "duplicate_keys": len(self.duplicates),
                "duplicate_events": sum(max(0, int(r.get("count", 1)) - 1) for r in self.duplicates.values()),
                "top_duplicates": dup_rows[:16],
            },
            "recent_tail_events": list(self.recent_events),
        }

    def write_dashboard(self) -> None:
        try:
            atomic_json(self.path, self.summary())
        except Exception as e:
            log("phantom_obs", f"observer dashboard write failed: {e}")

    def line(self) -> str:
        obj = self.summary()
        groups = obj.get("groups", {})
        janus = groups.get("janus_phantom", {})
        random_control = groups.get("random_control", {})
        dup = obj.get("duplicate_submit_watch", {})
        return (
            f"observer janus_gex={float(janus.get('fresh_tailgex',0.0)):.3f} "
            f"random_gex={float(random_control.get('fresh_tailgex',0.0)):.3f} "
            f"top_recipe={((obj.get('top_exact_recipes') or [{}])[0]).get('name','none')} "
            f"dup_events={dup.get('duplicate_events',0)}"
        )
...
def choose_phantom_janus_task(args: argparse.Namespace, proofmind: JanusProofMind, endurance: Any, memory: KombuchaMemory, dual_lock: DualLockMemory, rng: random.Random, cfgs: List[BuildConfig], round_id: int, worker_id: int) -> Tuple[str, int, BuildConfig, str]:
    cfg = next((c for c in cfgs if c.name == "canonical"), cfgs[0])
    weights = [(k, w) for k, w in normalized_lane_weights(args) if k != "random_baseline"]
    total = sum(float(w) for _, w in weights)
    if total <= 0:
        weights = [("janus_dispatcher", 1.0)]
        total = 1.0
    x = rng.random()
    acc = 0.0
    lane = weights[-1][0]
    ...
    if lane == "linear_proof":
        return "linear", (round_id * 3 + worker_id) % SECTORS, cfg, "phantom_janus:linear_proof"
    if lane == "dual_lock":
        st, sec, cfg2, sublane = dual_lock.choose(rng, cfg, getattr(args, "dual_lock_linear_s6_weight", 40.0), getattr(args, "dual_lock_zim_s6_weight", 35.0), getattr(args, "dual_lock_knight_s11_weight", 25.0))
        return st, sec, cfg2, f"phantom_janus:{sublane}"
    if lane == "zim_reverse_s6":
        return "zim_reverse", 6, cfg, "phantom_janus:zim_reverse_s6"

    st, sec, cfg2 = proofmind.choose(memory, rng, cfgs, round_id, worker_id)
    if not getattr(args, "disable_endurance_oracle", False):
        st, sec, cfg2 = endurance.maybe_adjust_choice(st, sec, cfg2, rng, cfgs)
    return st, sec, cfg2, "phantom_janus:janus_dispatcher"


def choose_phantom_task(args: argparse.Namespace, proofmind: JanusProofMind, endurance: Any, memory: KombuchaMemory, dual_lock: DualLockMemory, rng: random.Random, cfgs: List[BuildConfig], round_id: int, worker_id: int, poolday: Dict[str, Any]) -> Tuple[str, int, BuildConfig, str]:
    cfg = next((c for c in cfgs if c.name == "canonical"), cfgs[0])
    if bool(getattr(args, "phantom_50_50", True)) and ((round_id + worker_id) % 2 == 1):
        return "random", rng.randrange(SECTORS), cfg, "random_baseline"

    if not bool(getattr(args, "phantom_50_50", True)):
        base = choose_v31_task(args, proofmind, endurance, memory, dual_lock, rng, cfgs, round_id, worker_id)
        if base[3] == "random_baseline":
            return base
    else:
        base = choose_phantom_janus_task(args, proofmind, endurance, memory, dual_lock, rng, cfgs, round_id, worker_id)

    if bool(getattr(args, "disable_theta", False)):
        return base
    return phantom_theta_choice(base, poolday, rng, cfg)
...
def append_accepted_index(proof_dir: str, proof_path: str, cand: Dict[str, Any]) -> None:
    ...
    try:
        ...
        entry = {
            ...
            "poolday_mode": cand.get("poolday_mode") or ((cand.get("poolday") or {}).get("poolday_mode") if isinstance(cand.get("poolday"), dict) else None),
            "theta_bucket": cand.get("theta_bucket") if cand.get("theta_bucket") is not None else ((cand.get("poolday") or {}).get("theta_bucket") if isinstance(cand.get("poolday"), dict) else None),
            "q_bucket": cand.get("q_bucket") if cand.get("q_bucket") is not None else ((cand.get("poolday") or {}).get("q_bucket") if isinstance(cand.get("poolday"), dict) else None),
            "network_pressure": ((cand.get("poolday") or {}).get("network_pressure") if isinstance(cand.get("poolday"), dict) else None) or ((cand.get("netpulse_snapshot") or {}).get("network_pressure") if isinstance(cand.get("netpulse_snapshot"), dict) else None),
            "submit_ack_ms": cand.get("netpulse_submit_ack_ms"),
            "rpc_rtt_avg_ms": ((cand.get("netpulse_snapshot") or {}).get("rpc_rtt_avg_ms") if isinstance(cand.get("netpulse_snapshot"), dict) else None),
            "socket_idle_ms": ((cand.get("netpulse_snapshot") or {}).get("socket_idle_ms") if isinstance(cand.get("netpulse_snapshot"), dict) else None),
        }
...
def dump_accepted_proof(proof_dir: str, cand: Dict[str, Any]) -> Optional[str]:
    try:
        ...
        try:
            ...
            if PHANTOM_OBSERVER_PACK is not None:
                PHANTOM_OBSERVER_PACK.observe_accepted(cand)
        except Exception as e:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA10_2V34FullPhantomPoolDayNetPulse")
    ...
    p.add_argument("--csv-log", default="rblganul_a10_2_v34_full_phantom_netpulse_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a10_2_v34_full_phantom_netpulse_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a10_2_v34_full_phantom_netpulse.json", help="periodic/exit Full Phantom NetPulse session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a10_2_v34_full_phantom_netpulse_best_brain.json", help="persistent Full Phantom NetPulse ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a10_2_v34_full_phantom_netpulse_dashboard.json", help="A10.2/Full Phantom NetPulse live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a10_2_v34_full_phantom_netpulse_zim_stride_memory.json")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a10_2_v34_full_phantom_netpulse_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a10_2_v34_full_phantom_netpulse_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a10_2_v34_full_phantom_netpulse_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a10_2_v34_full_phantom_netpulse_50_50_accounting.json",
                   help="fresh-only Phantom JANUS-vs-random-control accounting dashboard")
    p.add_argument("--disable-a9-accounting", action="store_true",
                   help="disable Phantom accounting layer; scheduler/wire are unchanged")
    p.add_argument("--a9-min-random-control-mh", type=float, default=250.0,
                   help="comparison gate waits until random_control has this many checked MH")
    ...
    p.add_argument("--phantom-50-50", dest="phantom_50_50", action="store_true", default=True,
                   help="split workers evenly between Phantom JANUS traversal and random_baseline control")
    p.add_argument("--no-phantom-50-50", dest="phantom_50_50", action="store_false",
                   help="disable equal exposure and use the legacy lane weights")
    p.add_argument("--disable-poolday", action="store_true",
                   help="disable PoolDay context telemetry/guidance; wire remains frozen")
    p.add_argument("--disable-theta", action="store_true",
                   help="disable Ramanujan theta phase guidance; PoolDay telemetry can still be written")
    p.add_argument("--phantom-theta-terms", type=int, default=12,
                   help="number of q-series terms used for theta2/theta3/theta4 phase telemetry")
    p.add_argument("--phantom-observer-path", default="rblganul_a10_2_v34_full_phantom_netpulse_observer_pack.json",
                   help="A10.2 full observer-pack dashboard JSON")
    p.add_argument("--disable-phantom-observer-pack", action="store_true",
                   help="disable exact recipe / FreshTailGEX / duplicate-risk observer pack")
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="optional legacy import; default OFF for clean A10.2 evidence")
    p.add_argument("--no-import-v30-state", dest="import_v30_state", action="store_false")
    p.add_argument("--v30-proof-dashboard", default="")
    p.add_argument("--v30-session-summary", default="")
    p.add_argument("--v30-janus-brain", default="")
    p.add_argument("--v30-stride-memory", default="")
    ...
    p.add_argument("--io-run-name", default="A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_CLEAN_50_50_R2_AFTER_NETPULSE_FIX", help="subfolder for this clean Full Phantom NetPulse run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_CLEAN_50_50_R2_AFTER_NETPULSE_FIX") or "A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_CLEAN_50_50_R2_AFTER_NETPULSE_FIX")
    ...
    defaults = {
        "csv_log": "rblganul_a10_2_v34_full_phantom_netpulse_lab.csv",
        "lockbox": "a10_2_v34_full_phantom_netpulse_lockbox.json",
        "session_summary": "session_summary_a10_2_v34_full_phantom_netpulse.json",
        "janus_brain": "rblganul_a10_2_v34_full_phantom_netpulse_best_brain.json",
        "proof_dashboard": "rblganul_a10_2_v34_full_phantom_netpulse_dashboard.json",
        "stride_memory": "rblganul_a10_2_v34_full_phantom_netpulse_zim_stride_memory.json",
        "strategy_rates": "rblganul_a10_2_v34_full_phantom_netpulse_strategy_rates.json",
        "tail_events": "rblganul_a10_2_v34_full_phantom_netpulse_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a10_2_v34_full_phantom_netpulse_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a10_2_v34_full_phantom_netpulse_50_50_accounting.json",
        "phantom_observer_path": "rblganul_a10_2_v34_full_phantom_netpulse_observer_pack.json",
    }
    for attr, name in defaults.items():
        if v31_io_was_default_path(getattr(args, attr, ""), name):
            setattr(args, attr, str(run_dir / name))

    # A10.2 is clean by default: do not auto-wire old counters unless explicitly requested.
    if bool(getattr(args, "import_v30_state", False)):
        ...
        args.v30_proof_dashboard = ""
        args.v30_session_summary = ""
        args.v30_janus_brain = ""
        args.v30_stride_memory = ""
        args.v30_proofs_dir = ""
    ...
    info = {
        ...
        "phantom_observer_path": getattr(args, "phantom_observer_path", ""),
        "v30_import": {
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A10_2_V34_FULL_PHANTOM_POOLDAY_NETPULSE_IO_SINGLE.py")
    log("phantom", f"50_50={args.phantom_50_50} poolday_enabled={not args.disable_poolday} theta_enabled={not args.disable_theta} theta_terms={args.phantom_theta_terms}")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("phantom", f"accounting_enabled={not args.disable_a9_accounting} dashboard={args.a9_accounting_dashboard} min_random_mh={args.a9_min_random_control_mh} min_fresh_proofs={args.a9_min_fresh_proofs}")
        log("phantom", f"50_50={args.phantom_50_50} poolday_enabled={not args.disable_poolday} theta_enabled={not args.disable_theta} theta_terms={args.phantom_theta_terms}")
        log("phantom_obs", f"observer_pack_enabled={not args.disable_phantom_observer_pack} path={args.phantom_observer_path}")
    ...
    global V31_RATEBOOK, V31_TAIL_TRACKER, V31_DUALLOCK_MEMORY, A9_ACCOUNTING, PHANTOM_CONTEXT, PHANTOM_OBSERVER_PACK
    ...
    PHANTOM_CONTEXT = PoolDayThetaContext(args.phantom_theta_terms)
    A9_ACCOUNTING = None if bool(args.disable_a9_accounting) else PhantomPoolDayThetaAccounting(args.a9_accounting_dashboard)
    PHANTOM_OBSERVER_PACK = None if bool(args.disable_phantom_observer_pack) else FullPhantomObserverPack(args.phantom_observer_path)
    ...
    update_session_state(
        ...
        phantom_accounting_dashboard=args.a9_accounting_dashboard,
        phantom_accounting_enabled=not bool(args.disable_a9_accounting),
        phantom_observer_path=args.phantom_observer_path,
        phantom_observer_enabled=not bool(args.disable_phantom_observer_pack),
        phantom_50_50=bool(args.phantom_50_50),
        poolday_enabled=not bool(args.disable_poolday),
        theta_enabled=not bool(args.disable_theta),
    )
    ...
    try:
        ...
        if PHANTOM_OBSERVER_PACK is not None:
            PHANTOM_OBSERVER_PACK.write_dashboard()
    ...
    with cf.ProcessPoolExecutor(max_workers=workers) as pool:
        while True:
            ...
            net_target = compact_bits_to_target(job.nbits)
            net_z = target_to_z_approx(net_target) if net_target else 0.0
            netpulse_snapshot = client.netpulse_snapshot()
            if PHANTOM_CONTEXT is not None and not bool(getattr(args, "disable_poolday", False)):
                poolday_snapshot = PHANTOM_CONTEXT.snapshot(job, client.pool_diff, round_id, netpulse_snapshot)
            else:
                poolday_snapshot = {
                    "enabled": False,
                    "job_id": job.job_id,
                    "job_seq": int(job.seq),
                    "round_id": int(round_id),
                    "poolday_mode": "POOLDAY_DISABLED",
                    "theta_bucket": 0,
                    "q_bucket": 0,
                    "netpulse": netpulse_snapshot,
                    "network_pressure": str(netpulse_snapshot.get("network_pressure", "OK")),
                }
            ...
            if prelog_round:
                log(
                    ...
                    f"poolday={poolday_snapshot.get('poolday_mode')} theta_b={poolday_snapshot.get('theta_bucket')} "
                    f"netpulse={netpulse_snapshot.get('network_pressure')} submit_ack_avg={float(netpulse_snapshot.get('submit_ack_avg_ms',0.0)):.1f}ms",
                )

            tasks: List[MineTask] = []
            for wid in range(workers):
                st, sec, cfg, lane = choose_phantom_task(args, proofmind, endurance, memory, V31_DUALLOCK_MEMORY, rng, cfgs, round_id, wid, poolday_snapshot)
            ...
            if PHANTOM_OBSERVER_PACK is not None:
                for _r in results:
                    PHANTOM_OBSERVER_PACK.observe_result(_r)
            ...
            if should_summary:
                ...
                if A9_ACCOUNTING is not None:
                    log("phantom", A9_ACCOUNTING.line())
                if PHANTOM_OBSERVER_PACK is not None:
                    log("phantom_obs", PHANTOM_OBSERVER_PACK.line())
                log("netpulse", client.netpulse_line())
            ...
            update_session_state(
                ...
                phantom_accounting_dashboard=getattr(args, "a9_accounting_dashboard", ""),
                phantom_accounting=A9_ACCOUNTING.summary(args, client, endurance, proofmind).get("comparison_gate", {}) if A9_ACCOUNTING is not None else {},
                phantom_observer_path=getattr(args, "phantom_observer_path", ""),
                phantom_observer=PHANTOM_OBSERVER_PACK.summary().get("duplicate_submit_watch", {}) if PHANTOM_OBSERVER_PACK is not None else {},
                netpulse=client.netpulse_snapshot(),
                v32_network_recovery=V32_NETWORK_RECOVERY.snapshot(client) if V32_NETWORK_RECOVERY is not None else {},
            ...
            csvlog.write_round(
                {
                    ...
                    "poolday_mode": poolday_snapshot.get("poolday_mode", ""),
                    "theta_bucket": poolday_snapshot.get("theta_bucket", ""),
                    "network_pressure": client.netpulse_snapshot().get("network_pressure", ""),
                    "submit_ack_avg_ms": f"{float(client.netpulse_snapshot().get('submit_ack_avg_ms', 0.0)):.3f}",
                    "rpc_rtt_avg_ms": f"{float(client.netpulse_snapshot().get('rpc_rtt_avg_ms', 0.0)):.3f}",
                    "socket_idle_ms": client.netpulse_snapshot().get("socket_idle_ms", 0),
                    "read_loop_gap_ms": client.netpulse_snapshot().get("read_loop_gap_ms", 0),
                    "pending_submit_age_max_ms": f"{float(client.netpulse_snapshot().get('pending_submit_age_max_ms', 0.0)):.3f}",
                    "bytes_in": client.netpulse_snapshot().get("bytes_in", 0),
                    "bytes_out": client.netpulse_snapshot().get("bytes_out", 0),
                    "submitted": client.submitted,
                    "accepted": client.accepted,
                    "rejected": client.rejected,
                }

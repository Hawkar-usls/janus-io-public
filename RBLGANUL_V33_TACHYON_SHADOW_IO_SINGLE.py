...
Run on PC:
  python RBLGANUL_V33_TACHYON_SHADOW_IO_SINGLE.py
...
VERSION = "Rblganul V33 Tachyon Shadow IO SINGLE 20260528_IOPATH"
SENTINEL = "RBLGANUL_V33_TACHYON_SHADOW_IO_SINGLE_20260528_IOPATH"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulV33"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulV33-TachyonShadow"
...
class TachyonShadowMode:
    ...
    def observe_accepted(self, cand: Dict[str, Any]) -> None:
        try:
            ...
            self.best_z = max(self.best_z, z)
            self.updated_at_utc = utc_stamp_iso()
            self.recent.append({"kind": "accepted", "key": row["key"], "z": z, "ts_utc": self.updated_at_utc})
        except Exception as e:
            try:
    ...
    def dashboard(self, args: argparse.Namespace, client: Any, round_id: int, total_hps: float, best_z: int, proofmind: JanusProofMind) -> Dict[str, Any]:
        ...
        return {
            ...
            "v33_tachyon_allocator": {
                "enabled": False,
                "reason": "Shadow Mode only in V33-A; allocator intentionally disabled",
                "wire_change_required": False,
            },
        }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulV33TachyonShadowOracle")
    ...
    p.add_argument("--csv-log", default="rblganul_v33_tachyon_shadow_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="v33_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_v33.json", help="periodic/exit V33 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_v33_best_brain.json", help="persistent V33 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_v33_tachyon_shadow_dashboard.json", help="V33 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_v33_zim_stride_memory.json")
    ...
    p.add_argument("--strategy-rates", default="rblganul_v33_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_v33_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_v33_dual_lock_memory.json")
    ...
    p.add_argument("--tachyon-shadow-path", default="rblganul_v33_tachyon_shadow.json",
    ...
    p.add_argument("--io-run-name", default="A7_V33_TACHYON_SHADOW_AFTER_V32", help="subfolder for this V33 Tachyon Shadow handoff run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A7_V33_TACHYON_SHADOW_AFTER_V32") or "A7_V33_TACHYON_SHADOW_AFTER_V32")
    ...
    previous_dashboard = v31_io_first_existing([
        old_roots[0] / "rblganul_v32_networkrecovery_dashboard.json",
        old_roots[1] / "rblganul_v31_duallock_dashboard.json",
        old_roots[1] / "proof_dashboard.json",
        old_roots[2] / "proof_dashboard.json",
        old_roots[3] / "proof_dashboard.json",
        old_roots[4] / "a3_janus_full_proof_dashboard.json",
        old_roots[5] / "a4_dual_lock_test_proof_dashboard.json",
        old_roots[6] / "a1_linear_pure_proof_dashboard.json",
    ])
    previous_summary = v31_io_first_existing([
        old_roots[0] / "session_summary_v32.json",
        old_roots[1] / "session_summary_v31.json",
        old_roots[1] / "session_summary.json",
        old_roots[2] / "session_summary.json",
        old_roots[3] / "session_summary.json",
        old_roots[4] / "a3_janus_full_session_summary.json",
        old_roots[5] / "a4_dual_lock_test_session_summary.json",
        old_roots[6] / "a1_linear_pure_session_summary.json",
    ])
    previous_brain = v31_io_first_existing([
        old_roots[0] / "rblganul_v32_best_brain.json",
        old_roots[1] / "rblganul_v31_best_brain.json",
        old_roots[1] / "rblganul_v30_best_brain.json",
        old_roots[2] / "rblganul_v30_best_brain.json",
        old_roots[3] / "rblganul_v30_best_brain.json",
        old_roots[4] / "a3_janus_full_best_brain.json",
        old_roots[5] / "a4_dual_lock_test_best_brain.json",
    ])
    previous_stride = v31_io_first_existing([
        old_roots[0] / "rblganul_v32_zim_stride_memory.json",
        old_roots[1] / "rblganul_v31_zim_stride_memory.json",
        ...
        old_roots[4] / "a3_janus_full_stride_memory.json",
        old_roots[5] / "a4_dual_lock_test_stride_memory.json",
    ])
    ...
    defaults = {
        "csv_log": "rblganul_v33_tachyon_shadow_lab.csv",
        "lockbox": "v33_lockbox.json",
        "session_summary": "session_summary_v33.json",
        "janus_brain": "rblganul_v33_best_brain.json",
        "proof_dashboard": "rblganul_v33_tachyon_shadow_dashboard.json",
        "stride_memory": "rblganul_v33_zim_stride_memory.json",
        "strategy_rates": "rblganul_v33_strategy_rates.json",
        "tail_events": "rblganul_v33_tail_events.jsonl",
        "dual_lock_memory": "rblganul_v33_dual_lock_memory.json",
        "tachyon_shadow_path": "rblganul_v33_tachyon_shadow.json",
    }
    ...
    info = {
        "schema": "v33-io-path-bootstrap-1",
        "version": VERSION,
        "sentinel": SENTINEL,
        "created_at_utc": utc_stamp_iso(),
    ...
    try:
        atomic_json(str(run_dir / "v33_io_bootstrap.json"), info)
    except Exception:
        pass
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_V33_TACHYON_SHADOW_IO_SINGLE.py")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("io", f"bootstrap={Path(args.lockbox).parent / 'v33_io_bootstrap.json'}")
        return

    global V31_RATEBOOK, V31_TAIL_TRACKER, V31_DUALLOCK_MEMORY, V33_TACHYON_SHADOW
    ...
    with cf.ProcessPoolExecutor(max_workers=workers) as pool:
        while True:
            ...
            update_session_state(
                ...
                v33_tachyon_allocator="disabled_shadow_only",
            )

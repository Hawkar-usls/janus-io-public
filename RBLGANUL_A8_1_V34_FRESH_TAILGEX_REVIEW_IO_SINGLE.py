...
VERSION = "Rblganul A8.1/V34 FreshTailGEX Review IO SINGLE 20260601_FRESH_ONLY"
SENTINEL = "RBLGANUL_A8_1_V34_FRESH_TAILGEX_REVIEW_IO_SINGLE_20260601_FRESH_ONLY"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulV34-FreshTailGEXReview"
...
class DuplicateSubmitGuard:
    """A8.1 pre-submit duplicate guard.
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            "schema": "a8-1-duplicate-submit-guard-1",
            "enabled": True,
...
class FreshTailGEXReview:
    """A8.1 / V34 fresh-only TailGEX review layer.
    ...
    SCHEMA = "a8-1-v34-fresh-tailgex-review-1"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA8_1V34FreshTailGEXReviewOracle")
    ...
    p.add_argument("--csv-log", default="rblganul_a8_1_v34_fresh_tailgex_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a8_1_v34_fresh_tailgex_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a8_1_v34_fresh_tailgex.json", help="periodic/exit A8.1/V34 fresh-tailgex session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a8_1_v34_best_brain.json", help="persistent A8.1/V34 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a8_1_v34_fresh_tailgex_dashboard.json", help="A8.1/V34 fresh-tailgex live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a8_1_v34_zim_stride_memory.json")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a8_1_v34_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a8_1_v34_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a8_1_v34_dual_lock_memory.json")
    ...
    p.add_argument("--tachyon-shadow-path", default="rblganul_a8_1_v34_tachyon_shadow.json",
    ...
    p.add_argument("--allocator-review-path", default="rblganul_a8_1_v34_allocator_review.json",
    ...
    p.add_argument("--enable-fresh-tailgex-review", dest="enable_fresh_tailgex_review", action="store_true", default=True,
                   help="A8.1/V34: enable fresh-only TailGEX review; observe-only, no scheduler/wire changes")
    p.add_argument("--disable-fresh-tailgex-review", dest="enable_fresh_tailgex_review", action="store_false",
                   help="A8.1/V34: disable fresh-only TailGEX review")
    p.add_argument("--fresh-tailgex-path", default="rblganul_a8_1_v34_fresh_tailgex_review.json",
                   help="A8.1/V34 fresh-only TailGEX review JSON")
    p.add_argument("--fresh-tailgex-window-seconds", type=int, default=21600,
                   help="FreshTailGEX window in seconds; default 6 hours")
    p.add_argument("--fresh-tailgex-window-count", type=int, default=2000,
                   help="FreshTailGEX window accepted-proof count cap")
    p.add_argument("--fresh-tailgex-min-lane-accepted", type=int, default=50,
                   help="minimum fresh accepted proofs for a lane before fresh_gex is trusted")
    p.add_argument("--fresh-champion-exposure-floor", type=float, default=0.15,
                   help="champion watch low-exposure threshold, e.g. 0.15 = 15%")
    p.add_argument("--fresh-tailgex-stale-hours", type=float, default=6.0,
    ...
    p.add_argument("--io-run-name", default="A8_1_V34_FRESH_TAILGEX_REVIEW_AFTER_A8", help="subfolder for this A8.1/V34 fresh-tailgex review run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A8_1_V34_FRESH_TAILGEX_REVIEW_AFTER_A8") or "A8_1_V34_FRESH_TAILGEX_REVIEW_AFTER_A8")
    run_dir = (root / run_name).resolve()
    proofs_dir = run_dir / "proofs"
    run_dir.mkdir(parents=True, exist_ok=True)
    proofs_dir.mkdir(parents=True, exist_ok=True)

    # Discover previous artifacts before changing CWD. A8.1 must prefer the last A8
    ...
    previous_dashboard = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_1_v34_fresh_tailgex_dashboard.json",
    ...
    previous_summary = v31_io_first_existing([
        ...
        a8_previous_root / "session_summary_a8_1_v34_fresh_tailgex.json",
    ...
    previous_brain = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_1_v34_best_brain.json",
    ...
    previous_stride = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_1_v34_zim_stride_memory.json",
    ...
    defaults = {
        "csv_log": "rblganul_a8_1_v34_fresh_tailgex_lab.csv",
        "lockbox": "a8_1_v34_fresh_tailgex_lockbox.json",
        "session_summary": "session_summary_a8_1_v34_fresh_tailgex.json",
        "janus_brain": "rblganul_a8_1_v34_best_brain.json",
        "proof_dashboard": "rblganul_a8_1_v34_fresh_tailgex_dashboard.json",
        "stride_memory": "rblganul_a8_1_v34_zim_stride_memory.json",
        "strategy_rates": "rblganul_a8_1_v34_strategy_rates.json",
        "tail_events": "rblganul_a8_1_v34_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a8_1_v34_dual_lock_memory.json",
        "tachyon_shadow_path": "rblganul_a8_1_v34_tachyon_shadow.json",
        "allocator_review_path": "rblganul_a8_1_v34_allocator_review.json",
        "fresh_tailgex_path": "rblganul_a8_1_v34_fresh_tailgex_review.json",
    }
    ...
    info = {
        "schema": "a8-1-v34-fresh-tailgex-io-path-bootstrap-1",
        "version": VERSION,
        "sentinel": SENTINEL,
        "created_at_utc": utc_stamp_iso(),
    ...
    try:
        atomic_json(str(run_dir / "a8_1_v34_io_bootstrap.json"), info)
    except Exception:
        pass
    ...
    for cand in candidates:
        ...
        if V34_DUPLICATE_SUBMIT_GUARD is not None:
            ...
            if not allowed:
                ...
                dump_json("rblganul_a8_1_v34_duplicate_submit_drop.json", cand)
                continue
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A8_1_V34_FRESH_TAILGEX_REVIEW_IO_SINGLE.py")
    ...
    log("fresh_tailgex", f"A8.1/V34 FreshTailGEX Review enabled={args.enable_fresh_tailgex_review}; FRESH-ONLY REVIEW, allocator=OFF, wire=FROZEN, path={getattr(args, 'fresh_tailgex_path', '')} window={getattr(args, 'fresh_tailgex_window_seconds', 0)}s/{getattr(args, 'fresh_tailgex_window_count', 0)} proofs")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("io", f"bootstrap={Path(args.lockbox).parent / 'a8_1_v34_io_bootstrap.json'}")
        return

    global V31_RATEBOOK, V31_TAIL_TRACKER, V31_DUALLOCK_MEMORY, V33_TACHYON_SHADOW, V33_ALLOCATOR_REVIEW, V34_FRESH_TAILGEX_REVIEW, V34_DUPLICATE_SUBMIT_GUARD
    ...
    V34_FRESH_TAILGEX_REVIEW = FreshTailGEXReview(

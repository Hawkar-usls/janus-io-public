...
# -*- coding: utf-8 -*-
"""
RBLGANUL A9.2 / V32 RANDOMIZED TRAVERSAL MIRROR STRICT 50_50 SINGLE
2026-06-04
...
Run on PC:
  python RBLGANUL_A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_IO_SINGLE.py

A9.2 experiment:
  Strict 50/50 comparison of the original V32 broad JANUS mixture against a
  randomized traversal mirror. The mirror uses the same JANUS-shaped lane and
  sector scaffolds, but its nonce traversal strategy stays random. No wire,
  header, submit, prevhash, extranonce, allocator, or ProofMind steering is
  added to the mirror side.
...
A9.2 note:
  V32 scheduler/wire behavior is preserved. A9.2 adds a randomized traversal
  mirror: 50% checked work goes to the original broad JANUS mixture, and 50%
  goes to random nonce traversal inside the same lane/sector scaffold. The
  mirror side does not use ProofMind/HUNT steering; it tests whether the scaffold
...
VERSION = "Rblganul A9.2 V32 Randomized Traversal Mirror Strict 50_50 IO SINGLE 20260604"
SENTINEL = "RBLGANUL_A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_IO_SINGLE_20260604"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulA9_2"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulA9_2-V32RandomMirror5050"
...
class A9RandomControlAccounting:
    """Fresh-only A9.2 accounting layer.

    This class does not choose tasks and does not touch header/nonce/submit wire.
    It only groups the V32 lane mix into:
      - janus_broad_mixture: original adaptive JANUS lanes
      - randomized_traversal_mirror: random nonce traversal inside JANUS-shaped cells
    """
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-2-v32-randomized-traversal-mirror-strict-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
            "fresh_started_at_utc": self.started_at_utc,
            "fresh_uptime_seconds": max(0.0, time.time() - self.started_wall),
            "objective": "Strict 50/50 comparison: V32 broad JANUS mixture versus randomized traversal mirror under the same job stream and desktop-load run.",
            "wire_change_required": False,
...
def choose_randomized_traversal_mirror_task(
    ...
    This is the A9.2 control: it borrows the traversal shape from JANUS, then
    ...
    if lane == "linear_proof":
        ...
        return "random", sector, cfg, "random_mirror:linear_proof"
    ...
    if lane == "dual_lock":
        ...
        if branch == "knight_s11":
            return "random", 11, cfg, "random_mirror:dual_lock_knight_s11"
        if branch == "zim_reverse_s6":
            return "random", 6, cfg, "random_mirror:dual_lock_zim_reverse_s6"
        return "random", 6, cfg, "random_mirror:dual_lock_linear_s6"

    if lane == "zim_reverse_s6":
        return "random", 6, cfg, "random_mirror:zim_reverse_s6"

    sector = (int(round_id) * 7 + int(worker_id) * 5 + rng.randrange(SECTORS)) % SECTORS
    return "random", sector, cfg, "random_mirror:janus_dispatcher"
...
def choose_v31_task(args: argparse.Namespace, proofmind: JanusProofMind, endurance: Any, memory: KombuchaMemory, dual_lock: DualLockMemory, rng: random.Random, cfgs: List[BuildConfig], round_id: int, worker_id: int) -> Tuple[str, int, BuildConfig, str]:
    cfg = next((c for c in cfgs if c.name == "canonical"), cfgs[0])
    if bool(getattr(args, "strict_50_50_random_control", True)):
        # Strict comparison mode: half of equal-size worker tasks use randomized
        # traversal inside JANUS-shaped cells, half keep the original broad JANUS
        # mixture. With an odd worker count the parity flips every round, so
        # checked MH stays balanced over time.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA9_2V32RandomizedTraversalMirror5050")
    ...
    p.add_argument("--csv-log", default="rblganul_a9_2_v32_randomized_traversal_mirror_50_50_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a9_2_v32_randomized_traversal_mirror_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a9_2_v32_randomized_traversal_mirror.json", help="periodic/exit A9.2 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a9_2_v32_best_brain.json", help="persistent A9.2 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a9_2_v32_randomized_traversal_mirror_dashboard.json", help="A9.2/V32 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a9_2_v32_zim_stride_memory.json")
    p.add_argument("--disable-notify-oracle", action="store_true")
    p.add_argument("--notify-pause-window-ms", type=int, default=450)
    # V31 DualLock Oracle flags. A9.2 strict mode uses these as internal JANUS-half
    # proportions, then gives the randomized traversal mirror the other 50%.
    p.add_argument("--strict-50-50-randomized-traversal-mirror", dest="strict_50_50_random_control", action="store_true", default=True,
                   help="default ON: deterministic 50% V32 broad JANUS mixture vs 50% randomized JANUS-shaped traversal mirror")
    p.add_argument("--no-strict-50-50-randomized-traversal-mirror", dest="strict_50_50_random_control", action="store_false",
                   help="disable strict split and fall back to weighted A9.2 lane scheduling")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a9_2_v32_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a9_2_v32_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a9_2_v32_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a9_2_v32_randomized_traversal_mirror_accounting.json",
                   help="fresh-only A9.2 strict 50/50 JANUS-vs-randomized-traversal-mirror accounting dashboard")
    ...
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="import older dashboard/session/proof index at startup; default OFF for clean A9.2 evidence")
    ...
    p.add_argument("--io-run-name", default="A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_AFTER_A9_1", help="subfolder for this A9.2 randomized traversal mirror run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_AFTER_A9_1") or "A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_AFTER_A9_1")
    ...
    defaults = {
        "csv_log": "rblganul_a9_2_v32_randomized_traversal_mirror_50_50_lab.csv",
        "lockbox": "a9_2_v32_randomized_traversal_mirror_lockbox.json",
        "session_summary": "session_summary_a9_2_v32_randomized_traversal_mirror.json",
        "janus_brain": "rblganul_a9_2_v32_best_brain.json",
        "proof_dashboard": "rblganul_a9_2_v32_randomized_traversal_mirror_dashboard.json",
        "stride_memory": "rblganul_a9_2_v32_zim_stride_memory.json",
        "strategy_rates": "rblganul_a9_2_v32_strategy_rates.json",
        "tail_events": "rblganul_a9_2_v32_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a9_2_v32_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a9_2_v32_randomized_traversal_mirror_accounting.json",
    }
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A9_2_V32_RANDOMIZED_TRAVERSAL_MIRROR_IO_SINGLE.py")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("a9", f"accounting_enabled={not args.disable_a9_accounting} dashboard={args.a9_accounting_dashboard} min_mirror_mh={args.a9_min_random_control_mh} min_fresh_proofs={args.a9_min_fresh_proofs}")

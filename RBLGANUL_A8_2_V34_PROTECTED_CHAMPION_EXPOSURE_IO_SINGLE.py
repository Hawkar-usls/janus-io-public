...
VERSION = "Rblganul A8.2/V34 Protected Champion Exposure IO SINGLE 20260601_PROTECTED_FLOOR"
SENTINEL = "RBLGANUL_A8_2_V34_PROTECTED_CHAMPION_EXPOSURE_IO_SINGLE_20260601_PROTECTED_FLOOR"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulV34-ProtectedChampionExposure"
...
class ProtectedExposureReview:
    """A8.2 / V34 fresh-only TailGEX review layer.
    ...
    SCHEMA = "a8-2-v34-protected-exposure-review-1"
    ...
    def _key(self, lane: Any, strategy: Any, sector: Any, cfg_name: Any, stride_arm: Any = None) -> str:
        ...
        arm = self._i(stride_arm, -999) if stride_arm is not None else -999
        if arm == -999:
            return f"{lane_s}::{st_s}/s{sec}/{cfg_s}"
        return f"{lane_s}::{st_s}/s{sec}/{cfg_s}/a{arm}"
    ...
    def _fresh_rows(self) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        ...
        out.sort(key=lambda r: (float(r.get("fresh_gex", 0.0)), int(r.get("fresh_best_z", 0)), int(r.get("fresh_accepted", 0)), float(r.get("fresh_exposure", 0.0))), reverse=True)
        return out, meta
    ...
    def snapshot(self, shadow: Optional[TachyonShadowMode] = None, client: Optional[Any] = None) -> Dict[str, Any]:
        ...
        obj = {
            ...
            "rules": {
                ...
                "do_not_change": ["header", "nonce", "extranonce2", "ntime", "nbits", "prevhash", "TruthGate", "scheduler_weights"],
            },
        }
        self.last_snapshot = obj
        return obj
    ...
    def line(self) -> str:
        ...
        return (
            ...
            f"top_fresh={top_s}{champ_s} allocator=OFF wire=FROZEN"
        )
...
class ProtectedChampionExposureFloor:
    ...
    SCHEMA = "a8-2-v34-protected-champion-exposure-floor-1"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA8_2V34ProtectedChampionExposureOracle")
    ...
    p.add_argument("--csv-log", default="rblganul_a8_2_v34_protected_exposure_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a8_2_v34_protected_exposure_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a8_2_v34_protected_exposure.json", help="periodic/exit A8.2/V34 protected-exposure session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a8_2_v34_best_brain.json", help="persistent A8.2/V34 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a8_2_v34_protected_exposure_dashboard.json", help="A8.2/V34 protected-exposure live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a8_2_v34_zim_stride_memory.json")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a8_2_v34_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a8_2_v34_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a8_2_v34_dual_lock_memory.json")
    ...
    p.add_argument("--tachyon-shadow-path", default="rblganul_a8_2_v34_tachyon_shadow.json",
    ...
    p.add_argument("--allocator-review-path", default="rblganul_a8_2_v34_allocator_review.json",
    ...
    p.add_argument("--enable-protected-exposure-review", dest="enable_fresh_tailgex_review", action="store_true", default=True,
                   help="A8.2/V34: enable fresh-only TailGEX review; observe-only, no scheduler/wire changes")
    p.add_argument("--disable-protected-exposure-review", dest="enable_fresh_tailgex_review", action="store_false",
                   help="A8.2/V34: disable fresh-only TailGEX review")
    p.add_argument("--protected-exposure-path", "--fresh-tailgex-path", dest="fresh_tailgex_path", default="rblganul_a8_2_v34_protected_exposure_review.json",
                   help="A8.2/V34 fresh-only TailGEX review JSON")
    ...
    p.add_argument("--enable-protected-exposure", dest="enable_protected_exposure", action="store_true", default=True,
                   help="A8.2: enable protected checked-exposure floor test; allocator remains OFF and wire remains FROZEN")
    ...
    p.add_argument("--protected-zim-s6-floor", type=float, default=0.12,
                   help="A8.2 real checked-exposure floor for zim_reverse/s6/canonical, e.g. 0.12 = 12%")
    p.add_argument("--protected-knight-s11-floor", type=float, default=0.05,
                   help="A8.2 keep-alive checked-exposure floor for knight/s11/canonical, e.g. 0.05 = 5%")
    p.add_argument("--protected-deficit-boost", type=float, default=2.25,
                   help="A8.2 soft enforcement boost applied to total floor deficit")
    p.add_argument("--protected-max-force-prob", type=float, default=0.45,
                   help="A8.2 max probability per worker task to force an under-floor lane")
    p.add_argument("--protected-log-every-rounds", type=int, default=10,
                   help="A8.2 log protected exposure summary every N rounds")
    ...
    p.add_argument("--io-run-name", default="A8_2_V34_PROTECTED_CHAMPION_EXPOSURE_AFTER_A8_1", help="subfolder for this A8.2/V34 protected champion exposure run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A8_2_V34_PROTECTED_CHAMPION_EXPOSURE_AFTER_A8_1") or "A8_2_V34_PROTECTED_CHAMPION_EXPOSURE_AFTER_A8_1")
    ...
    previous_dashboard = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_2_v34_protected_exposure_dashboard.json",
    ...
    previous_summary = v31_io_first_existing([
        ...
        a8_previous_root / "session_summary_a8_2_v34_protected_exposure.json",
    ...
    previous_brain = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_2_v34_best_brain.json",
    ...
    previous_stride = v31_io_first_existing([
        ...
        a8_previous_root / "rblganul_a8_2_v34_zim_stride_memory.json",
    ...
    defaults = {
        "csv_log": "rblganul_a8_2_v34_protected_exposure_lab.csv",
        "lockbox": "a8_2_v34_protected_exposure_lockbox.json",
        "session_summary": "session_summary_a8_2_v34_protected_exposure.json",
        "janus_brain": "rblganul_a8_2_v34_best_brain.json",
        "proof_dashboard": "rblganul_a8_2_v34_protected_exposure_dashboard.json",
        "stride_memory": "rblganul_a8_2_v34_zim_stride_memory.json",
        "strategy_rates": "rblganul_a8_2_v34_strategy_rates.json",
        "tail_events": "rblganul_a8_2_v34_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a8_2_v34_dual_lock_memory.json",
        "tachyon_shadow_path": "rblganul_a8_2_v34_tachyon_shadow.json",
        "allocator_review_path": "rblganul_a8_2_v34_allocator_review.json",
        "fresh_tailgex_path": "rblganul_a8_2_v34_protected_exposure_review.json",
    }
    ...
    try:
        atomic_json(str(run_dir / "a8_2_v34_io_bootstrap.json"), info)
    except Exception:
        pass
    ...
    for cand in candidates:
        ...
        if V34_DUPLICATE_SUBMIT_GUARD is not None:
            ...
            if not allowed:
                ...
                dump_json("rblganul_a8_2_v34_duplicate_submit_drop.json", cand)
                continue
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A8_2_V34_PROTECTED_CHAMPION_EXPOSURE_IO_SINGLE.py")
    ...
    log("fresh_tailgex", f"A8.2/V34 Protected Champion Exposure review enabled={args.enable_fresh_tailgex_review}; FRESH-ONLY REVIEW, allocator=OFF, wire=FROZEN, path={getattr(args, 'fresh_tailgex_path', '')} window={getattr(args, 'fresh_tailgex_window_seconds', 0)}s/{getattr(args, 'fresh_tailgex_window_count', 0)} proofs")
    log("protected_exposure", f"A8.2 floor test enabled={getattr(args, 'enable_protected_exposure', True)} allocator=OFF wire=FROZEN zim_floor={getattr(args, 'protected_zim_s6_floor', 0.12):.3f} knight_floor={getattr(args, 'protected_knight_s11_floor', 0.05):.3f} max_force_prob={getattr(args, 'protected_max_force_prob', 0.45):.2f}")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("io", f"bootstrap={Path(args.lockbox).parent / 'a8_2_v34_io_bootstrap.json'}")
        return

    global V31_RATEBOOK, V31_TAIL_TRACKER, V31_DUALLOCK_MEMORY, V33_TACHYON_SHADOW, V33_ALLOCATOR_REVIEW, V34_FRESH_TAILGEX_REVIEW, V34_PROTECTED_EXPOSURE, V34_DUPLICATE_SUBMIT_GUARD

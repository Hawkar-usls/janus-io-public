...
# -*- coding: utf-8 -*-
"""
RBLGANUL A9.6 / V32 MINIMAL BUNNYHOP WITCHHUNTER STRICT 50_50 SINGLE
2026-06-05
...
Run on PC:
  python RBLGANUL_A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_IO_SINGLE.py
...
A9.6 note:
  V32 scheduler/wire behavior is preserved. A9.6 keeps the minimal BunnyHop
...
VERSION = "Rblganul A9.6 V32 Minimal BunnyHop WitchHunter Strict 50_50 IO SINGLE 20260605"
SENTINEL = "RBLGANUL_A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_IO_SINGLE_20260605"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulA9_6"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulA9_6-V32WitchHunter5050"
...
class A96BunnyHopAccounting:
    """Fresh-only A9.6 BunnyHop accounting layer.
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-6-v32-minimal-bunnyhop-witchhunter-strict-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
...
class WitchHunter:
    ...
    @staticmethod
    def group_name(lane: Any, strategy: Any = None) -> str:
        return A96BunnyHopAccounting.group_name(lane, strategy)

    @staticmethod
    def lane_key(lane: Any, strategy: Any, sector: Any, cfg_name: Any) -> str:
        return A96BunnyHopAccounting.lane_key(lane, strategy, sector, cfg_name)
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-6-witchhunter-dark-tail-observer-1",
...
class EnduranceOracle:
    ...
    def line(self) -> str:
        return (
            f"hps_ewmaРІвЂ°в‚¬{self.hps_ewma:,.0f} best_hpsРІвЂ°в‚¬{self.best_hps_ewma:,.0f} "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA9_6V32BunnyHopTachyonScoutWake5050")
    ...
    p.add_argument("--csv-log", default="rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a9_6_v32_minimal_bunnyhop_witchhunter_50_50_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a9_6_v32_minimal_bunnyhop_witchhunter_50_50.json", help="periodic/exit A9.6 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a9_6_v32_best_brain.json", help="persistent A9.6 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_dashboard.json", help="A9.6/V32 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a9_6_v32_zim_stride_memory.json")
    p.add_argument("--disable-notify-oracle", action="store_true")
    p.add_argument("--notify-pause-window-ms", type=int, default=450)
    # V31 DualLock Oracle flags. A9.6 strict mode uses these as internal JANUS-half
    ...
    p.add_argument("--no-strict-50-50-randomized-traversal-mirror", dest="strict_50_50_random_control", action="store_false",
                   help="disable strict split and fall back to weighted A9.6 lane scheduling")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a9_6_v32_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a9_6_v32_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a9_6_v32_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_accounting.json",
                   help="fresh-only A9.6 strict 50/50 BunnyHop-JANUS-vs-randomized-traversal-mirror accounting dashboard")
    p.add_argument("--disable-a9-accounting", action="store_true",
                   help="disable A9 accounting layer; scheduler/wire are unchanged")
    p.add_argument("--witchhunter-dashboard", default="rblganul_a9_6_v32_witchhunter_dark_tail.json",
                   help="observer-only rejected/stale/drop dark-tail dashboard JSON")
    p.add_argument("--witchhunter-events", default="rblganul_a9_6_v32_witchhunter_dark_tail_events.jsonl",
    ...
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="import older dashboard/session/proof index at startup; default OFF for clean A9.6 evidence")
    ...
    p.add_argument("--io-run-name", default="A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_AFTER_A9_5", help="subfolder for this A9.6 BunnyHop scout/wake run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_AFTER_A9_5") or "A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_AFTER_A9_5")
    ...
    defaults = {
        "csv_log": "rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_lab.csv",
        "lockbox": "a9_6_v32_minimal_bunnyhop_witchhunter_50_50_lockbox.json",
        "session_summary": "session_summary_a9_6_v32_minimal_bunnyhop_witchhunter_50_50.json",
        "janus_brain": "rblganul_a9_6_v32_best_brain.json",
        "proof_dashboard": "rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_dashboard.json",
        "stride_memory": "rblganul_a9_6_v32_zim_stride_memory.json",
        "strategy_rates": "rblganul_a9_6_v32_strategy_rates.json",
        "tail_events": "rblganul_a9_6_v32_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a9_6_v32_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a9_6_v32_minimal_bunnyhop_witchhunter_50_50_accounting.json",
        "witchhunter_dashboard": "rblganul_a9_6_v32_witchhunter_dark_tail.json",
        "witchhunter_events": "rblganul_a9_6_v32_witchhunter_dark_tail_events.jsonl",
    }
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A9_6_V32_MINIMAL_BUNNYHOP_WITCHHUNTER_50_50_IO_SINGLE.py")
    ...
    A9_ACCOUNTING = None if bool(args.disable_a9_accounting) else A96BunnyHopAccounting(args.a9_accounting_dashboard)
    ...
    if args.auto_escalate_local_z or args.lowdiff_jump_to_floor:
        log("autodiff", f"V24 submit gate starts at local_submit_z={local_submit_z}; auto_escalate={args.auto_escalate_local_z} lowdiff_jump={args.lowdiff_jump_to_floor} floor_diff={args.lowdiff_floor_diff:g} floor_zРІвЂ°в‚¬{floor_z_hint}")
    else:
        log("autodiff", f"NoEscalate mode={args.mode}: requested_local_submit_z={local_submit_z}; auto_escalate=False lowdiff_jump=False. Effective submit gate still follows the pool target, so if pool_diff=65536 then effective_zРІвЂ°в‚¬48 and z<48 will not be submitted.")
        ...
        while True:
            ...
            if prelog_round:
                log(
                    ...
                    f"pool_zРІвЂ°в‚¬{pool_z:.2f} requested_local_z={local_submit_z} effective_submit_z={effective_submit_z} "
                    f"effective_diffРІвЂ°в‚¬{z_to_difficulty(effective_submit_z):.6g} "
                    f"net_zРІвЂ°в‚¬{net_z:.2f} tasks={workers} batch={batch:,}",
                )
                ...
                if rej_delta > 0:
                    ...
                    if args.auto_escalate_local_z:
                        ...
                        if args.lowdiff_jump_to_floor and floor_z > local_submit_z:
                            ...
                            reason = f"jump_to_pool_floor diffРІвЂ°в‚¬{args.lowdiff_floor_diff:g}"
                        else:
                        ...
                        if local_submit_z != old_z:
                            ...
                            log("autodiff", f"Difficulty too low x{rej_delta}: {reason}; local_submit_z {old_z}->{local_submit_z} local_diffРІвЂ°в‚¬{approx_diff:.6g}")
                            eta = fmt_duration(expected_share_seconds(max(1.0, approx_diff), max(1.0, total_hps)))
                            log("autodiff", f"at current hpsРІвЂ°в‚¬{total_hps:,.0f}, expected share at local_diffРІвЂ°в‚¬{approx_diff:.6g} is about {eta}")
            ...
            if should_summary:
                ...
                log(
                    "summary",
                    f"round={round_id} checked={total_checked:,} hpsРІвЂ°в‚¬{total_hps:,.0f} "
                ...
                if total_hps > 0:
                    ...
                    log("eta", f"pool_eta@diff={client.pool_diff:g}РІвЂ°в‚¬{eta_pool}; gate_eta@z={active_gate_z}/diffРІвЂ°в‚¬{gate_diff:.6g}РІвЂ°в‚¬{eta_gate}; hpsРІвЂ°в‚¬{total_hps:,.0f}")

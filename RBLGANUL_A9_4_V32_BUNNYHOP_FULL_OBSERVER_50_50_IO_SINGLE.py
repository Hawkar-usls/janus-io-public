...
# -*- coding: utf-8 -*-
"""
RBLGANUL A9.4 / V32 BUNNYHOP FULL OBSERVER STRICT 50_50 SINGLE
2026-06-04
...
Run on PC:
  python RBLGANUL_A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_IO_SINGLE.py
...
A9.4 note:
  V32 scheduler/wire behavior is preserved. A9.4 keeps the BunnyHop cold-start
  gate: the JANUS half begins as random traversal inside JANUS-shaped cells,
  then wakes into broad JANUS only after fresh accepted-share evidence appears.
  The control half stays randomized traversal mirror for strict 50/50 accounting.
"""
...
VERSION = "Rblganul A9.4 V32 BunnyHop Full Observer Strict 50_50 IO SINGLE 20260604"
SENTINEL = "RBLGANUL_A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_IO_SINGLE_20260604"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulA9_4"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulA9_4-V32BunnyHopObserver5050"
            ...
            if mid in self.pending_submits:
                ...
                if msg.get("result") is True:
                    ...
                    try:
                        ...
                        if A94_OBSERVER_PACK is not None:
                            A94_OBSERVER_PACK.write()
                    except Exception as e:
...
class A93BunnyHopAccounting:
    """Fresh-only A9.4 BunnyHop accounting layer.
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-4-v32-bunnyhop-full-observer-strict-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
...
class A94FullObserverPack:
    """Passive observer pack for the next A9 line.

    It records PoolDay, Ramanujan theta phase, slime/atlas signatures, and
    desktop/network load context. It never chooses a nonce, changes a header,
    edits submit params, or touches scheduler weights.
    """

    def __init__(self, path: str, theta_terms: int = 12) -> None:
        self.path = path
        ...
        self.last_job_id = ""
        self.last_job_wall = 0.0
        self.job_intervals = deque(maxlen=128)
        self.last_net = None
        self.last_net_wall = 0.0
        self.rows: Dict[str, Dict[str, Any]] = {}
        self.accepted_rows: Dict[str, Dict[str, Any]] = {}
        self.tail_events = deque(maxlen=64)

    @staticmethod
    def _theta2(q: float, terms: int) -> float:
        return 2.0 * sum((q ** ((n + 0.5) ** 2)) for n in range(terms))

    @staticmethod
    def _theta3(q: float, terms: int) -> float:
        return 1.0 + 2.0 * sum((q ** (n * n)) for n in range(1, terms + 1))

    @staticmethod
    def _theta4(q: float, terms: int) -> float:
        return 1.0 + 2.0 * sum(((-1.0) ** n) * (q ** (n * n)) for n in range(1, terms + 1))

    def _row(self, key: str, kind: str) -> Dict[str, Any]:
    ...
    def _accepted_row(self, key: str, kind: str) -> Dict[str, Any]:
        if key not in self.accepted_rows:
            self.accepted_rows[key] = {
                "key": key,
                "kind": kind,
                "accepted": 0,
                "best_z": 0,
        ...
        return self.accepted_rows[key]

    @staticmethod
    def _touch_tail(row: Dict[str, Any], zbits: int) -> None:
        row["best_z"] = max(int(row.get("best_z", 0) or 0), int(zbits or 0))
        for z in (28, 30, 32, 33, 34, 36, 38):
            if int(zbits or 0) >= z:
                row[f"z{z}"] = int(row.get(f"z{z}", 0) or 0) + 1

    def pc_snapshot(self) -> Dict[str, Any]:
        snap: Dict[str, Any] = {
            ...
            "net_packets_per_sec": None,
            "net_err_drop_total": None,
            "gaming_hint": False,
            "game_process": "",
            "load_state": "PC_UNKNOWN",
        }
        if psutil is None:
            return snap
        try:
            cpu_total = float(psutil.cpu_percent(interval=None))
            per_core = list(psutil.cpu_percent(percpu=True, interval=None))
            mem = psutil.virtual_memory()
            snap["cpu_total_pct"] = cpu_total
            snap["idle_cores"] = sum(1 for x in per_core if float(x) < 30.0)
            snap["memory_pct"] = float(getattr(mem, "percent", 0.0))
            game_names = ("dayz", "steam", "arma", "cs2", "dota", "valorant", "minecraft", "arduino")
            for proc in psutil.process_iter(["name", "cpu_percent", "memory_percent"]):
                try:
                    name = str(proc.info.get("name") or "")
                    lname = name.lower()
                    if any(g in lname for g in game_names):
                        snap["gaming_hint"] = True
                        snap["game_process"] = name
                        break
                except Exception:
                    continue
            now = time.time()
            net = psutil.net_io_counters()
            err_drop = int(getattr(net, "errin", 0) or 0) + int(getattr(net, "errout", 0) or 0) + int(getattr(net, "dropin", 0) or 0) + int(getattr(net, "dropout", 0) or 0)
            snap["net_err_drop_total"] = err_drop
            if self.last_net is not None and self.last_net_wall > 0:
                dt = max(1e-6, now - self.last_net_wall)
                b0 = int(getattr(self.last_net, "bytes_sent", 0) or 0) + int(getattr(self.last_net, "bytes_recv", 0) or 0)
                b1 = int(getattr(net, "bytes_sent", 0) or 0) + int(getattr(net, "bytes_recv", 0) or 0)
                p0 = int(getattr(self.last_net, "packets_sent", 0) or 0) + int(getattr(self.last_net, "packets_recv", 0) or 0)
                p1 = int(getattr(net, "packets_sent", 0) or 0) + int(getattr(net, "packets_recv", 0) or 0)
                snap["net_bytes_per_sec"] = max(0.0, (b1 - b0) / dt)
                snap["net_packets_per_sec"] = max(0.0, (p1 - p0) / dt)
            self.last_net = net
            self.last_net_wall = now
            if snap["gaming_hint"]:
                snap["load_state"] = "PC_GAME_OR_TOOL"
            elif cpu_total >= 82.0:
                snap["load_state"] = "PC_BUSY"
            elif cpu_total >= 55.0:
                snap["load_state"] = "PC_MIXED"
            else:
                snap["load_state"] = "PC_CLEAR"
        except Exception as e:
            snap["error"] = str(e)
        return snap

    def round_snapshot(self, job: Any, client: Any, round_id: int) -> Dict[str, Any]:
        now = time.time()
        job_id = str(getattr(job, "job_id", "") or "")
        if job_id and job_id != self.last_job_id:
            if self.last_job_wall > 0:
                self.job_intervals.append(max(0.0, now - self.last_job_wall))
            self.last_job_id = job_id
            self.last_job_wall = now
        avg_job_interval = sum(self.job_intervals) / len(self.job_intervals) if self.job_intervals else 0.0
        pool_diff = float(getattr(client, "pool_diff", 0.0) or 0.0)
        pool_target = difficulty_to_target(pool_diff if pool_diff > 0 else 1.0)
        pool_z = target_to_z_approx(pool_target)
        net_target = compact_bits_to_target(str(getattr(job, "nbits", "") or ""))
        net_z = target_to_z_approx(net_target) if net_target else 0.0
        net_diff = (float(DIFF1_TARGET) / float(net_target)) if net_target else 0.0
        height_like = int(str(job_id)[-8:], 16) if job_id and all(c in "0123456789abcdefABCDEF" for c in str(job_id)[-8:]) else int(round_id)
        q_seed = (pool_z + net_z + (height_like % 257) / 32.0 + (round_id % 37) / 11.0)
        q = math.exp(-max(0.08, min(5.0, q_seed / 48.0)))
        t2 = self._theta2(q, self.theta_terms)
        t3 = self._theta3(q, self.theta_terms)
        t4 = self._theta4(q, self.theta_terms)
        theta_mix = (t2 * 0.37) + (t3 * 0.41) + (abs(t4) * 0.22)
        theta_bucket = int(abs(theta_mix * 1000.0)) % 8
        q_bucket = int(abs(q * 1000.0)) % 8
        pc = self.pc_snapshot()
        mode = "POOLDAY_NORMAL"
        if bool(pc.get("gaming_hint")):
            mode = "POOLDAY_DESKTOP_SHARED"
        elif avg_job_interval and avg_job_interval < 12.0:
            mode = "POOLDAY_FAST_JOBS"
        elif avg_job_interval and avg_job_interval > 80.0:
            mode = "POOLDAY_SLOW_JOBS"
        if str(pc.get("load_state")) in ("PC_BUSY", "PC_GAME_OR_TOOL"):
            mode = f"{mode}_LOAD"
        snap = {
            "schema": "a9-4-poolday-theta-pc-slime-observer-snapshot-1",
            "round": int(round_id),
            "job_id": job_id,
            "job_seq": int(getattr(job, "seq", 0) or 0),
            "clean": bool(getattr(job, "clean", False)),
            "pool_diff": pool_diff,
            "pool_z_approx": pool_z,
            "network_z_approx": net_z,
            ...
            "avg_job_interval_sec": avg_job_interval,
            "job_interval_samples": len(self.job_intervals),
            "q": q,
            "q_bucket": q_bucket,
    ...
    def atlas_signature(self, r: Any) -> Dict[str, Any]:
        nonce = int(getattr(r, "best_nonce", 0) or 0) & 0xFFFFFFFF
        field = stable_seed(getattr(r, "lane", ""), getattr(r, "strategy", ""), getattr(r, "sector", 0), getattr(r, "stride", 0), nonce) & 0xFFFFFFFF
        return {
            "atlas_field": int(field % 32),
            "atlas_mode": int((field >> 5) % 4),
            "atlas_phase": int((field >> 9) % 16),
            "atlas_stride_class": "stride" if int(getattr(r, "stride", 0) or 0) else "non_stride",
            "atlas_fossil_z": int(getattr(r, "best_z", 0) or 0),
        }

    def attach_candidate(self, cand: Dict[str, Any], snapshot: Dict[str, Any]) -> None:
        cand["a94_observer"] = {
            "poolday_mode": snapshot.get("poolday_mode"),
            "theta_bucket": snapshot.get("theta_bucket"),
            "q_bucket": snapshot.get("q_bucket"),
            "pc_load_state": (snapshot.get("pc") or {}).get("load_state"),
            "gaming_hint": (snapshot.get("pc") or {}).get("gaming_hint"),
            "network_difficulty_from_nbits": snapshot.get("network_difficulty_from_nbits"),
            "implied_network_hashrate_ehps": snapshot.get("implied_network_hashrate_ehps"),
            "avg_job_interval_sec": snapshot.get("avg_job_interval_sec"),
            "wire_change_required": False,
            "scheduler_effect": "observe_only",
        }

    def observe_result(self, r: Any, snapshot: Dict[str, Any]) -> None:
        checked = int(getattr(r, "checked", 0) or 0)
        zbits = int(getattr(r, "best_z", 0) or 0)
        atlas = self.atlas_signature(r)
        keys = [
            ("poolday", str(snapshot.get("poolday_mode", "POOLDAY_UNKNOWN"))),
            ("theta", f"theta_b{int(snapshot.get('theta_bucket', 0) or 0)}"),
            ("q", f"q_b{int(snapshot.get('q_bucket', 0) or 0)}"),
            ("pc", str((snapshot.get("pc") or {}).get("load_state", "PC_UNKNOWN"))),
            ("atlas", f"atlas_m{atlas['atlas_mode']}_f{atlas['atlas_field']}"),
        ]
        for kind, key in keys:
            row = self._row(key, kind)
            row["checked"] = int(row.get("checked", 0) or 0) + checked
            row["result_count"] = int(row.get("result_count", 0) or 0) + 1
            self._touch_tail(row, zbits)

    def observe_accepted(self, cand: Dict[str, Any]) -> None:
        zbits = int(cand.get("zbits", 0) or 0)
        obs = cand.get("a94_observer") if isinstance(cand.get("a94_observer"), dict) else {}
        keys = [
            ("poolday", str(obs.get("poolday_mode", "POOLDAY_UNKNOWN"))),
            ("theta", f"theta_b{int(obs.get('theta_bucket', 0) or 0)}"),
            ("q", f"q_b{int(obs.get('q_bucket', 0) or 0)}"),
            ("pc", str(obs.get("pc_load_state", "PC_UNKNOWN"))),
        ]
        for kind, key in keys:
            row = self._accepted_row(key, kind)
        ...
        if zbits >= 30:
            self.tail_events.append({
                ...
                "strategy": cand.get("strategy"),
                "sector": cand.get("sector"),
                "poolday_mode": obs.get("poolday_mode"),
                "theta_bucket": obs.get("theta_bucket"),
                "pc_load_state": obs.get("pc_load_state"),
                "nonce_submit_hex": cand.get("nonce_submit_hex"),
            })
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        for row in self.rows.values():
            out = dict(row)
            mh = float(out.get("checked", 0) or 0) / 1_000_000.0
            out["mh"] = mh
            out["best_z_per_mh_context"] = (float(out.get("best_z", 0) or 0) / mh) if mh > 0 else 0.0
            result_rows.append(out)
        accepted_rows = [dict(v) for v in self.accepted_rows.values()]
        result_rows.sort(key=lambda x: (int(x.get("best_z", 0)), int(x.get("z32", 0)), float(x.get("mh", 0.0))), reverse=True)
        accepted_rows.sort(key=lambda x: (int(x.get("best_z", 0)), int(x.get("z32", 0)), int(x.get("accepted", 0))), reverse=True)
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-4-v32-bunnyhop-full-observer-pack-1",
            "objective": "Strict 50/50 BunnyHop JANUS versus randomized traversal mirror with PoolDay/Theta/Slime/PC-load observers only.",
            "started_at_utc": self.started_at_utc,
            "written_at_utc": utc_stamp_iso(),
            ...
            "top_result_contexts": result_rows[:32],
            "top_accepted_contexts": accepted_rows[:32],
            "recent_tail_events": list(self.tail_events),
        }
    ...
    def write(self) -> None:
        try:
            ...
            log("a94", f"observer pack write failed: {e}")

    def line(self) -> str:
        snap = self.last_snapshot or {}
        pc = snap.get("pc") if isinstance(snap.get("pc"), dict) else {}
        return (
            f"poolday={snap.get('poolday_mode', 'n/a')} "
            f"theta_b={snap.get('theta_bucket', 'n/a')} q_b={snap.get('q_bucket', 'n/a')} "
            f"pc={pc.get('load_state', 'n/a')} game={pc.get('gaming_hint', False)}"
        )
...
def dump_accepted_proof(proof_dir: str, cand: Dict[str, Any]) -> Optional[str]:
    try:
        ...
        try:
            ...
            if A94_OBSERVER_PACK is not None:
                A94_OBSERVER_PACK.observe_accepted(cand)
                A94_OBSERVER_PACK.write()
        except Exception as e:
...
def write_lockbox(path: str, args: argparse.Namespace, cfgs: List[BuildConfig], workers: int, source_sha16: str = "") -> None:
    obj = {
        ...
        "a94_observer_path": getattr(args, "a94_observer_path", ""),
        "a94_observer_enabled": not bool(getattr(args, "disable_a94_observer_pack", False)),
        "a94_theta_terms": int(getattr(args, "a94_theta_terms", 12) or 12),
        "a94_observer_policy": "observe_only_no_scheduler_or_wire_effect",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA9_4V32BunnyHopFullObserver5050")
    ...
    p.add_argument("--csv-log", default="rblganul_a9_4_v32_bunnyhop_full_observer_50_50_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a9_4_v32_bunnyhop_full_observer_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a9_4_v32_bunnyhop_full_observer.json", help="periodic/exit A9.4 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a9_4_v32_best_brain.json", help="persistent A9.4 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a9_4_v32_bunnyhop_full_observer_dashboard.json", help="A9.4/V32 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a9_4_v32_zim_stride_memory.json")
    p.add_argument("--disable-notify-oracle", action="store_true")
    p.add_argument("--notify-pause-window-ms", type=int, default=450)
    # V31 DualLock Oracle flags. A9.4 strict mode uses these as internal JANUS-half
    ...
    p.add_argument("--no-strict-50-50-randomized-traversal-mirror", dest="strict_50_50_random_control", action="store_false",
                   help="disable strict split and fall back to weighted A9.4 lane scheduling")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a9_4_v32_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a9_4_v32_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a9_4_v32_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a9_4_v32_bunnyhop_full_observer_accounting.json",
                   help="fresh-only A9.4 strict 50/50 BunnyHop-JANUS-vs-randomized-traversal-mirror accounting dashboard")
    ...
    p.add_argument("--a94-observer-path", default="rblganul_a9_4_v32_full_observer_pack.json",
                   help="passive PoolDay/Theta/Slime/PC-load observer pack JSON")
    p.add_argument("--a94-theta-terms", type=int, default=12,
                   help="number of q-series terms used by passive Ramanujan theta observer")
    p.add_argument("--disable-a94-observer-pack", action="store_true",
                   help="disable A9.4 observer pack; scheduler and wire are unchanged either way")
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="import older dashboard/session/proof index at startup; default OFF for clean A9.4 evidence")
    ...
    p.add_argument("--io-run-name", default="A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_AFTER_A9_3", help="subfolder for this A9.4 BunnyHop full observer run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_AFTER_A9_3") or "A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_AFTER_A9_3")
    ...
    defaults = {
        "csv_log": "rblganul_a9_4_v32_bunnyhop_full_observer_50_50_lab.csv",
        "lockbox": "a9_4_v32_bunnyhop_full_observer_lockbox.json",
        "session_summary": "session_summary_a9_4_v32_bunnyhop_full_observer.json",
        "janus_brain": "rblganul_a9_4_v32_best_brain.json",
        "proof_dashboard": "rblganul_a9_4_v32_bunnyhop_full_observer_dashboard.json",
        "stride_memory": "rblganul_a9_4_v32_zim_stride_memory.json",
        "strategy_rates": "rblganul_a9_4_v32_strategy_rates.json",
        "tail_events": "rblganul_a9_4_v32_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a9_4_v32_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a9_4_v32_bunnyhop_full_observer_accounting.json",
        "a94_observer_path": "rblganul_a9_4_v32_full_observer_pack.json",
    }
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A9_4_V32_BUNNYHOP_FULL_OBSERVER_50_50_IO_SINGLE.py")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("a94", f"observer_enabled={not args.disable_a94_observer_pack} path={args.a94_observer_path} theta_terms={args.a94_theta_terms} scheduler_effect=observe_only")
        log("io", f"bootstrap={Path(args.lockbox).parent / 'v31_io_bootstrap.json'}")
        return

    global V31_RATEBOOK, V31_TAIL_TRACKER, V31_DUALLOCK_MEMORY, A9_ACCOUNTING, A94_OBSERVER_PACK
    ...
    A9_ACCOUNTING = None if bool(args.disable_a9_accounting) else A93BunnyHopAccounting(args.a9_accounting_dashboard)
    A94_OBSERVER_PACK = None if bool(args.disable_a94_observer_pack) else A94FullObserverPack(args.a94_observer_path, args.a94_theta_terms)
    ...
    update_session_state(
        ...
        a94_observer_path=args.a94_observer_path,
        a94_observer_enabled=not bool(args.disable_a94_observer_pack),
    )
    ...
    try:
        ...
        log("io", "initial dashboard/session/ratebook/a9-accounting/a94-observer files written")
    except Exception as e:
    ...
    with cf.ProcessPoolExecutor(max_workers=workers) as pool:
        while True:
            ...
            observer_snapshot = {}
            if A94_OBSERVER_PACK is not None:
                observer_snapshot = A94_OBSERVER_PACK.round_snapshot(job, client, round_id)
            ...
            while pending:
                ...
                for fut in done:
                    ...
                    if A94_OBSERVER_PACK is not None:
                        A94_OBSERVER_PACK.observe_result(r, observer_snapshot)
                        for _cand in r.candidates:
                            A94_OBSERVER_PACK.attach_candidate(_cand, observer_snapshot)
            ...
            if should_summary:
                ...
                if A94_OBSERVER_PACK is not None:
                    log("a94", A94_OBSERVER_PACK.line())
            ...
            update_session_state(
                ...
                a94_observer=A94_OBSERVER_PACK.summary() if A94_OBSERVER_PACK is not None else {},
                v32_network_recovery=V32_NETWORK_RECOVERY.snapshot(client) if V32_NETWORK_RECOVERY is not None else {},
            ...
            if A94_OBSERVER_PACK is not None and ((round_id % 5 == 0) or should_summary):
                A94_OBSERVER_PACK.write()
            if round_id % 100 == 0:
                write_session_summary(args.session_summary)

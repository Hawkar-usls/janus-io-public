...
# -*- coding: utf-8 -*-
"""
RBLGANUL A9.1 / V32 BROAD MIXTURE STRICT 50_50 RANDOM CONTROL SINGLE
2026-05-27
...
Run on PC:
  python RBLGANUL_A9_1_V32_BROAD_MIXTURE_STRICT_50_50_IO_SINGLE.py
...
A9.1 note:
  V32 scheduler/wire behavior is preserved. A9 adds fresh-only accounting that
  compares the existing broad JANUS lane mix against the existing random_baseline
  control lane under the same desktop-load conditions.
  A9.1 makes that comparison strict: 50% checked work goes to the broad JANUS
  mixture and 50% goes to pure random_control. The JANUS half keeps the original
  A9 internal broad-mixture proportions.
"""
...
VERSION = "Rblganul A9.1 V32 Broad-Mixture Strict 50_50 Random-Control IO SINGLE 20260604"
SENTINEL = "RBLGANUL_A9_1_V32_BROAD_MIXTURE_STRICT_50_50_IO_SINGLE_20260604"
...
DEFAULT_USER = "JANUS_PUBLIC_WORKER.RblganulA9_1"
DEFAULT_PASSWORD = "x"
...
DEFAULT_SUBSCRIBE_TAG = "NerdMinerV2/RblganulA9_1-V32BroadStrict5050"
...
class A9RandomControlAccounting:
    """Fresh-only A9 accounting layer.

    This class does not choose tasks and does not touch header/nonce/submit wire.
    It only groups the existing V32 lane mix into:
      - janus_broad_mixture: all non-control lanes
      - random_control: the explicit random_baseline lane
    """
    ...
    def summary(self, args: argparse.Namespace, client: Any, endurance: Any, proofmind: Any) -> Dict[str, Any]:
        ...
        janus = groups.get("janus_broad_mixture", self._decorated_row(self._base_row("janus_broad_mixture")))
        ...
        return {
            "version": VERSION,
            "sentinel": SENTINEL,
            "schema": "a9-1-v32-broad-mixture-strict-50-50-accounting-1",
            "written_at_utc": utc_stamp_iso(),
            "fresh_started_at_utc": self.started_at_utc,
            "fresh_uptime_seconds": max(0.0, time.time() - self.started_wall),
            "objective": "Strict 50/50 comparison: V32 broad JANUS mixture versus pure random_control under the same job stream and desktop-load run.",
            "wire_change_required": False,
            ...
            "scheduler_experiment": {
                ...
                "janus_half": "original A9 broad mixture internal proportions",
                "random_half": "pure random_baseline",
                "wire_touched": False,
            },
    ...
    def line(self) -> str:
        janus = self._decorated_row(self.groups.get("janus_broad_mixture", self._base_row("janus_broad_mixture")))
...
def broad_mixture_lane_weights(args: argparse.Namespace) -> List[Tuple[str, float]]:
    """Original A9 JANUS broad-mixture proportions, excluding random_control."""
    vals = [
...
def normalized_lane_weights(args: argparse.Namespace) -> List[Tuple[str, float]]:
    if bool(getattr(args, "strict_50_50_random_control", True)):
        ...
        return [(k, w * 0.50) for k, w in broad] + [("random_baseline", 0.50)]
    vals = [
        ("linear_proof", max(0.0, float(getattr(args, "linear_proof_weight", 35.0)))),
        ("janus_dispatcher", max(0.0, float(getattr(args, "janus_weight", 25.0)))),
        ("dual_lock", max(0.0, float(getattr(args, "dual_lock_weight", 20.0))) if getattr(args, "enable_dual_lock_lane", True) else 0.0),
...
def choose_v31_task(args: argparse.Namespace, proofmind: JanusProofMind, endurance: Any, memory: KombuchaMemory, dual_lock: DualLockMemory, rng: random.Random, cfgs: List[BuildConfig], round_id: int, worker_id: int) -> Tuple[str, int, BuildConfig, str]:
    cfg = next((c for c in cfgs if c.name == "canonical"), cfgs[0])
    if bool(getattr(args, "strict_50_50_random_control", True)):
        # Strict comparison mode: half of equal-size worker tasks are pure random,
        # half keep the original A9 broad JANUS mixture. With an odd worker count
        # the parity flips every round, so checked MH stays balanced over time.
        lane = "random_baseline" if ((int(round_id) + int(worker_id)) & 1) == 0 else pick_lane_from_weights(broad_mixture_lane_weights(args), rng)
    else:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="RblganulA9_1V32BroadMixtureStrict5050")
    ...
    p.add_argument("--csv-log", default="rblganul_a9_1_v32_broad_mixture_strict_50_50_lab.csv")
    p.add_argument("--proofs-dir", default="proofs", help="directory for accepted-share proof JSON files")
    p.add_argument("--lockbox", default="a9_1_v32_strict_50_50_lockbox.json", help="startup configuration lockbox JSON")
    p.add_argument("--session-summary", default="session_summary_a9_1_v32_broad_mixture_strict_50_50.json", help="periodic/exit A9.1 session summary JSON")
    p.add_argument("--janus-brain", default="rblganul_a9_1_v32_best_brain.json", help="persistent A9.1 ProofMind elite/bad memory")
    p.add_argument("--registry-dir", default="proofs", help="Janus meta-registry root; defaults to proofs/ so artifacts stay together")
    p.add_argument("--proof-dashboard", default="rblganul_a9_1_v32_broad_mixture_strict_50_50_dashboard.json", help="A9.1/V32 live dashboard JSON for long unattended runs")
    ...
    p.add_argument("--stride-memory", default="rblganul_a9_1_v32_zim_stride_memory.json")
    p.add_argument("--disable-notify-oracle", action="store_true")
    p.add_argument("--notify-pause-window-ms", type=int, default=450)
    # V31 DualLock Oracle flags. A9.1 strict mode uses these as internal JANUS-half
    # proportions, then gives pure random_control the other 50% of equal-size work.
    p.add_argument("--strict-50-50-random-control", dest="strict_50_50_random_control", action="store_true", default=True,
                   help="default ON: deterministic 50% V32 broad JANUS mixture vs 50% pure random_baseline")
    p.add_argument("--no-strict-50-50-random-control", dest="strict_50_50_random_control", action="store_false",
                   help="disable strict split and fall back to weighted A9 lane scheduling")
    ...
    p.add_argument("--strategy-rates", default="rblganul_a9_1_v32_strategy_rates.json")
    p.add_argument("--tail-events", default="rblganul_a9_1_v32_tail_events.jsonl")
    p.add_argument("--dual-lock-memory", default="rblganul_a9_1_v32_dual_lock_memory.json")
    p.add_argument("--a9-accounting-dashboard", default="rblganul_a9_1_v32_broad_mixture_strict_50_50_accounting.json",
                   help="fresh-only A9.1 strict 50/50 JANUS-vs-random-control accounting dashboard")
    ...
    p.add_argument("--import-v30-state", dest="import_v30_state", action="store_true", default=False, help="import older dashboard/session/proof index at startup; default OFF for clean A9.1 evidence")
    ...
    p.add_argument("--io-run-name", default="A9_1_V32_BROAD_MIXTURE_STRICT_50_50_AFTER_A9", help="subfolder for this A9.1 strict 50/50 run")
...
def v31_io_prepare_paths(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    run_name = str(getattr(args, "io_run_name", "A9_1_V32_BROAD_MIXTURE_STRICT_50_50_AFTER_A9") or "A9_1_V32_BROAD_MIXTURE_STRICT_50_50_AFTER_A9")
    ...
    defaults = {
        "csv_log": "rblganul_a9_1_v32_broad_mixture_strict_50_50_lab.csv",
        "lockbox": "a9_1_v32_strict_50_50_lockbox.json",
        "session_summary": "session_summary_a9_1_v32_broad_mixture_strict_50_50.json",
        "janus_brain": "rblganul_a9_1_v32_best_brain.json",
        "proof_dashboard": "rblganul_a9_1_v32_broad_mixture_strict_50_50_dashboard.json",
        "stride_memory": "rblganul_a9_1_v32_zim_stride_memory.json",
        "strategy_rates": "rblganul_a9_1_v32_strategy_rates.json",
        "tail_events": "rblganul_a9_1_v32_tail_events.jsonl",
        "dual_lock_memory": "rblganul_a9_1_v32_dual_lock_memory.json",
        "a9_accounting_dashboard": "rblganul_a9_1_v32_broad_mixture_strict_50_50_accounting.json",
    }
...
def main() -> None:
    ...
    log("run", "no long command needed: python RBLGANUL_A9_1_V32_BROAD_MIXTURE_STRICT_50_50_IO_SINGLE.py")
    ...
    if getattr(args, "selfcheck", False):
        ...
        log("a9", f"accounting_enabled={not args.disable_a9_accounting} dashboard={args.a9_accounting_dashboard} min_random_mh={args.a9_min_random_control_mh} min_fresh_proofs={args.a9_min_fresh_proofs}")
    ...
    A9_ACCOUNTING = None if bool(args.disable_a9_accounting) else A9RandomControlAccounting(args.a9_accounting_dashboard)
